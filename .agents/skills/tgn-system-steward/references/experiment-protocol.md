...
**运输缺失 ≠ 设计目标错误。** 如果上游已经有阶段背景、私人压力、机会价值、空间因果或成长因果，而某一种 whole-block / Long Block 注入方案失败，只能判该运输方案失败；不能反推这些背景不该存在。下一步应比较更窄、当前章绑定、reader-safe 的 transport 是否能保留目标，同时避免未来剧情或隐藏旧史泄漏。
