...
正文读感或经典对照评审先亲读相应经典原文的完整场景或连续窗口，记录真实阅读位置和范围，再读 TGN 输出。摘要、GBrain 卡、模型记忆不能代替这一步；经典原文不进入 Writer。
...
重要取舍给用户原始输出、试改及足够前后文，标明手工编辑处。用户反馈优先；未回复时继续暂定方案，不能记为接受。对白合并后还要看说话人是否能追踪、发言是否急促、人物是否有自然反应间隙；不能以短句数量或排比命中次数证明读感改善。

测试通过、局部手工编辑有效、短段被接受与自动 Batch 质量通过分别记录。生成改善要看实际自动链的连续正文；planning 层接近经典不能外推成成品同级，未测的跨书和长篇范围仍是 OPEN。
...
某种 whole-block 注入失败，只否定该处理方式；上游已有而下游缺失的当前私人压力、空间、机会价值和成长因果仍需按批准范围运输。不能把实现缺口当成删除设计目标的依据。解释压缩只删除静态边界的重复证明；动态 Growth、当前力量尺和 Situation / Reader Orientation 有新判断价值时仍须保留。
