...
DER=BASE/'derivation-l2-high-local'
OUT=BASE/'derivation-l2-high-local-blind'
...
def one(run,ch,judge):
 ...
 rnd=random.Random(f'L2-{run}-{ch}-{judge}');rnd.shuffle(names)
...
def main():
 ...
 summary={'schema_version':'local-authority-repair-l2-high-local-blind-v1','aggregates':agg,'rows':sorted(rows,key=lambda r:(r['run'],r['chapter'],r['judge']))}
