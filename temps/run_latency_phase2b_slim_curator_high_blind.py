...
EXP=BASE/'phase-2b-slim-curator-high'
J=BASE/'blind-judges-phase2b-slim-curator-high'
...
for ch in CHAPTERS:
 ...
 treatment=(tr/'final_slim_body.md').read_text(encoding='utf-8').strip()
 order=['control','treatment'];random.Random(2026082962+ch).shuffle(order);texts={'control':control,'treatment':treatment}
 key[str(ch)]={'A':order[0],'B':order[1],'control_sha256':hashlib.sha256(control.encode()).hexdigest(),'slim_treatment_sha256':hashlib.sha256(treatment.encode()).hexdigest()}
