...
BOOK = EXP_ROOT / "heldout-new-novel-2"
...
AUTHOR_DIRECTION = """成熟中文男频玄幻/仙侠长篇。作为第二部独立 held-out，必须与此前所有 derivation 明显不同：禁止九垂原/潮路/回潮楔/分身；禁止烬星洲、星力阶、远身、远控兵器或“第二战斗位置”作为核心幻想；禁止商队契约/物流治理作为主要发动机。

让当前系统自行生成新的世界、力量和人物。优先允许身体变化、异兽/伴生物、元素/空间、吞噬/炼化、奇物、遗迹、宗门/家族冲突等熟悉语言中的新作用，但不要强制某一类。要有简单公开力量主尺、具体值得抢的物/机会、强敌与人物关系。主角可以贪钱、好胜、好色/审美、爱面子、有野心或报复，不净化。前四章自然完成开篇抓力、核心能力第一次兑现、一个具体想拿的高价值对象、关系或敌意变化，以及下一层真实入口；Supporting Logic 不得成为 Story Engine。"""

NOVELTY_SEED = 202608302003
PROTOCOL_HASH = "E11BEFFE12F5016CA1DFB362631D3145212437A21C20BCA360B7D540C5E692E4"
def _section(text: str, heading: str) -> str:
    match = re.search(rf"(?ms)^#{2,3} {re.escape(heading)}\s*$\n(.*?)(?=^#{2,3} |\Z)", text)
    return match.group(1).strip() if match else ""


def _director_field(text: str, label: str) -> str:
    match = re.search(rf"(?m)^{re.escape(label)}：\s*(.+)$", text)
    if not match:
        raise RuntimeError(f"missing Director field {label}")
    return match.group(1).strip()


def final_facts_projection(chapter_number: int, director_response: str) -> str:
    approved_book = (BOOK / "BOOK.md").read_text(encoding="utf-8")
    power = (BOOK / "POWER_SEED.md").read_text(encoding="utf-8")
    releases = re.findall(rf"(?m)^- 第{chapter_number}章｜触发：(.+)$", approved_book)
    release_text = "；".join(item.strip() for item in releases) if releases else "NONE"
    core_power = _section(power, "一句话大白话") or "NONE"
    permanent_boundary = _section(power, "永久边界") or "NONE"
    return "\n".join([
        "## FINAL FACTS PROJECTION｜Runtime deterministic; no new story facts",
        f"Direct Result: {_director_field(director_response, '直接结果')}",
        f"State Change: {_director_field(director_response, '状态变化')}",
        f"Ending: {_director_field(director_response, '结尾推动力')}",
        f"Scheduled Reader Release: {release_text}",
        f"Core Power: {core_power}",
        f"Permanent Power Boundary: {permanent_boundary}",
    ])
...
def chapter(n: int) -> None:
    ...
    projection=final_facts_projection(n, dr)
    (d/'final_facts_projection.md').write_text(projection+'\n',encoding='utf-8')
...
def main():
    import hashlib
    protocol=(EXP_ROOT/'PROTOCOL_CANDIDATE_2.md').read_bytes()
