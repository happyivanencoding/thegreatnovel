...
PROTOCOL_HASH = "EEE2F39B13B48D0388CC29289C0E309BB731DEFECB7C15E69C6F0CD77045778D"
...
def one(run: str, chapter: int) -> dict:
    ...
    out = EXP / "derivation-heldout2-d2" / run / f"chapter-{chapter:04d}"
...
def main() -> None:
    actual = hashlib.sha256((EXP / "PROTOCOL_DERIVATION_D2.md").read_bytes()).hexdigest().upper()
    if actual != PROTOCOL_HASH:
        raise RuntimeError(f"D2 protocol changed after freeze: {actual}")
    rows = []
    ...
    n = len(rows)
    summary = {
        "schema_version": "medium-authority-watch-d2-derivation-v1",
    ...
    (EXP / "DERIVATION_D2_TIMING.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
