...
SOURCE = BOOK / "runs"
TREATMENT = ROOT / "books" / "real-exp-chapter-latency-innovation-20260829-v1" / "post-authority-reader-polish-all20"
OUT = ROOT / "books" / "real-exp-chapter-latency-innovation-20260829-v1" / "blind-post-authority-reader-polish-all20"
RUNNER = Path(r"C:\Users\jingx\AppData\Local\Temp\run-codex-acp.mjs")
CHAPTERS = (1, 3, 4, 5, 6, 10, 11, 12, 17, 18)
...
def prepare() -> dict:
    ...
    for chapter in CHAPTERS:
        ...
        order = ["control", "reader_polish"]
        random.Random(20260829320 + chapter).shuffle(order)
        texts = {"control": control, "reader_polish": treatment}
...
{texts[order[1]]}
"""
        authority_prompt = f"""你是匿名的 TGN Final Draft Authority 审稿人。两版共享同一 Frozen Mission、Curator、safe World、Reader Release、Power/Human Core、Canon 与上一章尾部。先检查两版是否事实等价：行动者/对象/顺序、Direct Result、State Change、Ending、资源/持有关系、时间窗口、精确力量、未知边界、Reader Release、Named Entity、情绪和关系事实都不能改变。事实等价后，再判断删除的句子是否真属重复总结，还是仍承担节奏、人物、关系、幻想或Payoff价值。
