...
SOURCE = BOOK / "runs"
OUT = ROOT / "books" / "real-exp-chapter-latency-innovation-20260829-v1" / "speculative-next-director-canonless"
...
def one(previous_chapter: int, next_chapter: int) -> dict:
    ...
    # State is running in parallel. Do not feed the Director a stale Canon snapshot:
    # the existing previous-tail block already contains the approved final prose and
    # remains the highest authority for immediate continuity.
    prompt = replace_h2(
        prompt,
        "当前 Canon Index",
        "（上一章 State Extraction 正在并行运行；不要使用旧 Canon 猜新状态。当前输入中的前文章末正式正文是已发生事实的最高来源。）",
    )
    prompt = replace_h2(
        prompt,
        "最近 1—3 章摘要",
        "（State 正在并行运行；本次不注入可能过期的摘要。只按当前章计划与前文章末正式正文承接。）",
    )
    prompt += "\n\n# STATE-PARALLEL DIRECTOR BOUNDARY\n\n上一章正式正文已经完成，State 只是压缩索引。不得因为缺少新 State 自行补造持有、力量、身份、伤势、知识或未兑现承诺；若前文章末没有明确支持某项状态，就保持未知或不写。"
