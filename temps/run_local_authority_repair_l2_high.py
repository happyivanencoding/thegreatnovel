...
OUT=ROOT/'books'/'real-exp-local-authority-repair-20260830-v1'/'derivation-l2-high-local'
RUNNER=Path(r'C:\Users\jingx\AppData\Local\Temp\run-codex-acp.mjs')
PROTOCOL_HASH=(ROOT/'books'/'real-exp-local-authority-repair-20260830-v1'/'PROTOCOL_L2_HIGH_LOCAL_SHA256.txt').read_text(encoding='utf-8').strip()
...
def call(prompt_path:Path,out_path:Path)->dict[str,Any]:
    proc=subprocess.run(['node',str(RUNNER),str(prompt_path),str(out_path),'gpt-5.6-luna','high',str(ROOT)],cwd=ROOT,text=True,capture_output=True,encoding='utf-8',errors='replace',timeout=900)
