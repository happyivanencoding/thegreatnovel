...
SOURCE = BOOK / "runs"
OUT = ROOT / "books" / "real-exp-chapter-latency-innovation-20260829-v1" / "speculative-next-director-v2"
...
def one(previous_chapter: int, next_chapter: int) -> dict:
    ...
    final_chapter = (BOOK / "chapters" / f"chapter-{previous_chapter:04d}.md").read_text(encoding="utf-8").strip()
    frozen_previous_mission = clean(
        (SOURCE / f"chapter-{previous_chapter:04d}" / "director_response.md").read_text(encoding="utf-8")
    )
    provisional = f"""# PROVISIONAL PREVIOUS-CHAPTER AUTHORITY｜State Extraction Parallel Input

State Extraction 正在与本次 Director 并行，因此下方旧 Canon Index 可能尚未写入上一章的新变化。上一章正式正文已经发生，是本轮最高事实来源；上一章 Frozen Mission 只作语义索引。只允许采用正文明确发生的事实，不从 Mission 未落实部分、合理推测或计划目标补 Canon。与旧 Canon 冲突时，以正式正文为准；不得把这一临时包扩写成新的世界/力量规则。

## PREVIOUS FROZEN MISSION
{frozen_previous_mission}

## PREVIOUS FINAL CHAPTER PROSE
{final_chapter}
"""
    prompt = prompt + "\n\n" + provisional
