...
RUNS = ("e2e-run4", "e2e-run5", "fresh-control-3", "fresh-control-4")
...
def candidate_text(run: str, sample: str, kind: str) -> str:
    ...
    return body(p.read_text(encoding="utf-8")).strip()
...
def one(sample: str, kind: str, judge: str) -> dict[str, Any]:
    out = BASE / "blinds-final-clean-v2b" / sample
    out.mkdir(parents=True, exist_ok=True)
    order = list(RUNS)
    random.Random(f"native-v2b-final-clean:{sample}:{kind}:{judge}").shuffle(order)
...
def main() -> None:
    ...
    summary = {
        "schema_version": "native-structured-final-clean-blind-v1",
        "runs": list(RUNS),
        "samples": len(SAMPLES),
    ...
    out = BASE / "blinds-final-clean-v2b"
