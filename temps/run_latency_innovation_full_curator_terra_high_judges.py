...
TREATMENT = ROOT / "books" / "real-exp-chapter-latency-innovation-20260829-v1" / "full-curator-terra-high-downstream"
OUT = ROOT / "books" / "real-exp-chapter-latency-innovation-20260829-v1" / "blind-full-curator-terra-high-downstream"
...
def prepare() -> dict:
    ...
    for chapter in CHAPTERS:
        ...
        pair = TREATMENT / f"chapter-{chapter:04d}"
        directory = OUT / f"chapter-{chapter:04d}"
        directory.mkdir(parents=True, exist_ok=True)
        candidates = {
            "luna_high_curator": (pair / "luna_high" / "final_body.md").read_text(encoding="utf-8").strip(),
            "terra_high_curator": (pair / "terra_high" / "final_body.md").read_text(encoding="utf-8").strip(),
        }
        order = ["luna_high_curator", "terra_high_curator"]
        random.Random(20260829920 + chapter).shuffle(order)
        ...
        reader_prompt = f"""你是匿名盲读的成熟中文男频长篇审稿人。两版共享同一冻结 Director、同一 Primary 与 Reviser 模型/effort，只更换了 Curator 模型；你不知道来源。只评最终正文，不按长度评分。
...
{candidates[order[1]]}
"""
        authority_judge_prompt = f"""你是匿名 TGN Authority / Canon 盲审员。两版最终正文共享同一 Frozen Mission、最终 Canon、World / Power / Human Authority、Reader Release 与 Ending；你不知道 Curator 模型。
