...
FORGE = BASE / "compilable_single_v4"
EXP = BASE / "downstream_S2_compilable_v4"
...
def assert_prompt_isolation(
    *,
    ...
    if failed:
        raise RuntimeError(f"compilable-v4 lane isolation failed: {failed}")
...
def stop_on_conflict(stage: str, text: str) -> None:
    ...
    dump(
        ...
        {
            "case": "fast_multiworld/S2_compilable_v4",
...
def main() -> None:
    ...
    protocol = """# Compilable Single-Pass S2 V4 Downstream Protocol
...
- All S1/S2/S3 cards must parse with World Interface, exact T0 Origin, exact Initial Scale Position, Power trigger/coverage/boundary and Authority-Compilation Trace.
...
- Power sees literal Ontology + exact Initial Scale Position + exact Power only; no Origin/Biography/Story.
- Human sees literal Ontology + exact T0 Origin + exact Initial Scale Position only; no special Power/Story.
...
"""
    ...
    world = load_or_run(
        ...
        model="gpt-5.6-luna",
        label="premise-compilable-v4-S2-world",
    )
    ...
    if power_path.exists() and human_path.exists():
        ...
        print("REUSE premise-compilable-v4-S2-power/human", flush=True)
    else:
        with ThreadPoolExecutor(max_workers=2) as pool:
            power_future = pool.submit(
                ...
                model="gpt-5.6-luna",
                label="premise-compilable-v4-S2-power",
            )
            human_future = pool.submit(
                ...
                model="gpt-5.6-luna",
                label="premise-compilable-v4-S2-human",
            )
    ...
    story = load_or_run(
        ...
        model="gpt-5.6-sol",
        label="premise-compilable-v4-S2-story",
    )
    ...
    outline = load_or_run(
        ...
        model="gpt-5.6-luna",
        label="premise-compilable-v4-S2-outline",
    )
    ...
    audit_prompt = f"""你是 TGN Authority-Compilable Premise 审计员。比较同一 fast-multiworld 作者方向下：原 S2 在 frozen V2 的真实 Story 层发生 Authority conflict；V3 加入 Authority-Compilation Trace 后又因精确 T0 尺位没有进入 Human lane 而 fail loud；V4 在同一次非 Canon Forge 中新增独立 Initial Scale Position 字段，并预注册 S2 后走完整真实链。只审计，不改稿。

必须判断：
1. 新 S2 的 Authority-Compilation Trace 是否逐项真实，还是自称闭合但依赖未授权机制。
2. World-only / World Interface / Ontology / T0 Origin / Initial Scale Position / Power trigger-coverage-boundary / Opening Promise 分别 PRESERVED / TRANSFORMED-BUT-PRESERVED / LOST / CONTRADICTED。
...
10. 对比 current baseline、四轴 C2、旧 S2 V1/V2/V3：结论 PASS / CONDITIONAL PASS / FAIL。只给一项会改变冻结判断的最小动作；不建议新增 Agent/Judge/Reviewer/Scorer/Hard Gate。

严格格式：
# AUTHORITY-COMPILABLE PREMISE V4 AUDIT
...
# V3 INITIAL SCALE POSITION CONFLICT
{read(BASE / 'downstream_S2_compilable_v3' / 'HUMAN_CANDIDATES.md')}
...
"""
    ...
    audit = load_or_run(
        ...
        model="gpt-5.6-terra",
        label="premise-compilable-v4-S2-audit",
    )
    dump(
        EXP / "RUN_SUMMARY.json",
        {
