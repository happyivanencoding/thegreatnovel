...
SOURCE = BOOK / "runs"
TREATMENT = ROOT / "books" / "real-exp-atomic-chapter-obligations-20260829-v1" / "phase-h-atomic-delta-corrected"
OUT = ROOT / "books" / "real-exp-atomic-chapter-obligations-20260829-v1" / "phase-i-blind-atomic-delta-corrected"
...
def prepare() -> dict:
    ...
    for chapter in CHAPTERS:
        ...
        treatment = (TREATMENT / f"chapter-{chapter:04d}" / "route_final_body.md").read_text(encoding="utf-8").strip()
        order = ["control", "atomic_delta"]
        random.Random(202608291450 + chapter).shuffle(order)
        texts = {"control": control, "atomic_delta": treatment}
        key[str(chapter)] = {"A": order[0], "B": order[1]}
        previous_tail = ""
        if chapter > 1:
            previous_tail = (BOOK / "chapters" / f"chapter-{chapter-1:04d}.md").read_text(encoding="utf-8")[-2200:]
        reader_prompt = f"""你是匿名的成熟中文男频长篇读者审稿人。两版来自同一冻结章节，不要猜模型/架构，不因字数长短自动评分。
...
{texts[order[1]]}
"""
        authority_prompt = f"""你是匿名的 TGN Final Draft Authority 审稿人。两版共享同一 Frozen Mission、Curator、safe World、Reader Release、Power/Human Core、Canon 与上一章尾部。先检查硬事实：行动者/对象/顺序、Direct Result、State Change、Ending、资源/持有关系、时间窗口、精确力量、未知边界、Reader Release、Named Entity；任何硬错不能被文笔抵消。再比较哪版在不越界的前提下更好地保护主角主动性、人物欲望、Payoff、Public Proof、关系和故事价值。
...
{(TREATMENT / f"chapter-{chapter:04d}" / "compact_obligations.md").read_text(encoding="utf-8")}
