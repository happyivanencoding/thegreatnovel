...
def candidates(run: str, chapter: int) -> dict[str, str]:
    ...
    watch = EXP / "derivation-heldout2-d3" / run / f"chapter-{chapter:04d}" / "medium_watch_final_body.md"
    return {
...
def one(run: str, chapter: int, judge: str) -> dict:
    ...
    names = list(texts)
    random.Random(f"d3:{run}:{chapter}:{judge}").shuffle(names)
    letters = "ABC"
...
{cand}"""
        model = "gpt-5.6-luna"
    out = EXP / "derivation-blind-d3" / run / f"chapter-{chapter:04d}"
...
def main():
    ...
    result={"schema_version":"medium-authority-watch-d3-blind-v1","aggregates":agg,"rows":rows}
    (EXP/"DERIVATION_D3_BLIND.json").write_text(json.dumps(result,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
