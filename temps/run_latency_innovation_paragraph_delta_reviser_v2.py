...
SOURCE = BOOK / "runs"
OUT = ROOT / "books" / "real-exp-chapter-latency-innovation-20260829-v1" / "paragraph-delta-reviser-v2"
...
def entity_candidates(text: str) -> set[str]:
    result: set[str] = set()
    for suffix, width in ENTITY_SPECS:
...
def nearest_authorized_entity(candidate: str, authority: str) -> str | None:
    suffix = next((suffix for suffix, _ in ENTITY_SPECS if candidate.endswith(suffix)), "")
    if not suffix:
        return None
    choices = [item for item in entity_candidates(authority) if item.endswith(suffix) and len(item) == len(candidate)]
    scored = sorted(
        ((difflib.SequenceMatcher(None, candidate, item).ratio(), item) for item in choices),
        reverse=True,
    )
...
def compile_payload(payload: str, old_text: str, authority: str) -> tuple[str, list[dict]]:
    compiled = payload
    repairs: list[dict] = []
    old_numbers = set(re.findall(r"\d+(?:\.\d+)?", old_text))
    authority_numbers = set(re.findall(r"\d+(?:\.\d+)?", authority))
    new_numbers = set(re.findall(r"\d+(?:\.\d+)?", compiled)) - old_numbers
    unauthorized_numbers = sorted(new_numbers - authority_numbers)
    if unauthorized_numbers:
        raise ValueError("unauthorized numbers=" + ",".join(unauthorized_numbers))

    old_entities = entity_candidates(old_text)
    authority_entities = entity_candidates(authority)
    for candidate in sorted(entity_candidates(compiled) - old_entities):
        if candidate in authority_entities:
            continue
        repaired = nearest_authorized_entity(candidate, authority)
        if repaired is None:
            raise ValueError("unauthorized named entity=" + candidate)
        compiled = compiled.replace(candidate, repaired)
        repairs.append({"from": candidate, "to": repaired})

    # Pronoun flips are global-continuity-sensitive. Let the existing full Reviser
    # handle them rather than accepting a local paragraph change.
    old_gender = (old_text.count("他"), old_text.count("她"))
    new_gender = (compiled.count("他"), compiled.count("她"))
    if old_gender != new_gender:
        raise ValueError(f"gender pronoun count changed {old_gender}->{new_gender}")
    return compiled, repairs


def apply_ops(response: str, primary: str, authority: str) -> tuple[str, list[dict], list[dict]]:
    text = clean(response)
    if text == "KEEP_ALL":
    ...
    for match in OP_PATTERN.finditer(text):
        ...
        if kind in {"REPLACE", "DELETE"}:
            ...
            occupied |= ids
            if kind == "REPLACE":
                payload, repairs = compile_payload(payload, "\n\n".join(original[start - 1:end]), authority)
                compiler_repairs.extend(repairs)
            ...
            payload, repairs = compile_payload(payload, "", authority)
            compiler_repairs.extend(repairs)
    ...
    return final, operations, compiler_repairs
...
def one(chapter: int) -> dict:
    ...
    fallback_error = ""
    try:
        final, operations, compiler_repairs = apply_ops(response, primary, authority_context)
    except Exception as error:
        fallback = True
    ...
    return {
        ...
        "effective_route_seconds": round(float(data.get("wall_seconds") or 0) + (float(control_data.get("wall_seconds") or 0) if fallback else 0.0), 3),
        "effective_route_speedup_percent": round((1 - (float(data.get("wall_seconds") or 0) + (float(control_data.get("wall_seconds") or 0) if fallback else 0.0)) / float(control_data.get("wall_seconds") or 1)) * 100, 2),
        "fallback": fallback,
