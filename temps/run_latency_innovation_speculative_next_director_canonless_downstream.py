...
DIRECTOR=ROOT/'books'/'real-exp-chapter-latency-innovation-20260829-v1'/'speculative-next-director-canonless'
OUT=ROOT/'books'/'real-exp-chapter-latency-innovation-20260829-v1'/'speculative-next-director-canonless-downstream'
RUNNER=Path(r'C:\Users\jingx\AppData\Local\Temp\run-codex-acp.mjs')
TRANSITIONS=((2,3),(3,4),(5,6),(12,13),(14,15),(18,19))
CHAPTERS=tuple(ch for _,ch in TRANSITIONS)
PREVIOUS={ch:prev for prev,ch in TRANSITIONS}
...
def call(pp:Path,out:Path,model:str,effort:str)->dict:
    # Resume already-completed stages after an interrupted experiment.
    if out.exists():
        try:
    ...
    for attempt in range(3):
        try:
            cp=subprocess.run(
                ['node',str(RUNNER),str(pp),str(out),model,effort,str(ROOT)],
                cwd=ROOT,text=True,capture_output=True,encoding='utf-8',errors='replace',
                timeout=900,
            )
        except subprocess.TimeoutExpired:
            last=f'timeout after 900s: {pp}'
...
def run_one(ch:int)->dict:
    ...
    new_director=clean((DIRECTOR/f'chapter-{ch:04d}'/'speculative_director_response.md').read_text(encoding='utf-8'))
    ...
    return {
      'chapter':ch,
      'previous_chapter':PREVIOUS[ch],
      'speculative_director_seconds':json.loads((DIRECTOR/f'chapter-{ch:04d}'/'speculative_director_acp.json').read_text(encoding='utf-8'))['wall_seconds'],
      'previous_state_seconds':json.loads((SOURCE/f'chapter-{PREVIOUS[ch]:04d}'/'state_acp.json').read_text(encoding='utf-8'))['wall_seconds'],
      ...
      'control_state_through_reviser_seconds':round(float(json.loads((SOURCE/f'chapter-{PREVIOUS[ch]:04d}'/'state_acp.json').read_text(encoding='utf-8'))['wall_seconds'])+sum(float(control[k]) for k in ('director','curator','primary','authority_reviser')),3),
      'treatment_parallel_state_through_reviser_seconds':round(max(float(json.loads((SOURCE/f'chapter-{PREVIOUS[ch]:04d}'/'state_acp.json').read_text(encoding='utf-8'))['wall_seconds']),float(json.loads((DIRECTOR/f'chapter-{ch:04d}'/'speculative_director_acp.json').read_text(encoding='utf-8'))['wall_seconds']))+sum(float(x or 0) for x in (cj.get('wall_seconds'),pj.get('wall_seconds'),rj.get('wall_seconds'))),3)
    }
