...
ROOT=Path(r'C:\dev\tgn-story-mvp');BOOK=ROOT/'books'/'real-exp-fast-world-20ch-20260828-v1';SOURCE=BOOK/'runs';T=ROOT/'books'/'real-exp-chapter-latency-innovation-20260829-v1'/'strict-reader-polish-fast20-replicate';OUT=ROOT/'books'/'real-exp-chapter-latency-innovation-20260829-v1'/'blind-strict-reader-polish-fast20-replicate';RUNNER=Path(r'C:\Users\jingx\AppData\Local\Temp\run-codex-acp.mjs');CH=(5,16,18)
...
for ch in CH:
 d=OUT/f'chapter-{ch:04d}';d.mkdir(parents=True,exist_ok=True);control=(BOOK/'chapters'/f'chapter-{ch:04d}.md').read_text(encoding='utf-8').strip();polish=(T/f'chapter-{ch:04d}'/'final_body.md').read_text(encoding='utf-8').strip();order=['control','polish'];random.Random(20260829270+ch).shuffle(order);texts={'control':control,'polish':polish};key[str(ch)]={'A':order[0],'B':order[1]}
