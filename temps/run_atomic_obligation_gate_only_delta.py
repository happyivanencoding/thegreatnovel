...
OUT = ROOT / "books" / "real-exp-atomic-chapter-obligations-20260829-v1" / "phase-c0-gate-only-delta"
...
def one(chapter: int) -> dict:
    ...
    # Keep the original Paragraph-Delta writing contract unchanged. Atomic
    # obligations are used only by deterministic adoption after local apply, so the
    # model is not encouraged to explain or realize the checklist in prose.
    prompt = (
        base.DELTA_TEMPLATE
        + "\n\n"
        + authority_context
