...
处理 handoff_id=handoff_e3812487bdb31ad27f2d4b61。

运行 deterministic workflow start；成功后信任其 RUNNING contract。
Python 已冻结 executor_skill=bootstrap-original-novel；调用 $bootstrap-original-novel Skill，严格执行该 Skill 的 requested_stage=FOUNDATION_DEVELOPMENT_PROPOSAL。
...
```json
{
  ...
  "effective_query": {
    "creative_problem": "FOUNDATION_DEVELOPMENT_PROPOSAL：围绕作者 premise 与已确认阅读体验寻找可迁移的故事机制，只作为 Core/Foundation/Candidate 的参考候选。",
    "creative_problem_tags": [],
    "max_cards": 6,
    "narrative_drives": [
      "SURVIVAL_RESOURCE",
