...
- 力量体系的正常值、稀缺度、境界/能力的可观察差距；先用 1—3 句普通话说清力量来源、直接作用、成长与失败，再给会反复使用的对象/层级短名。**Power Growth Causality 也是 World Root Authority**：World 必须让普通读者能复述“力量从哪里进入人/身体/器物；普通人每天具体怎样变强；什么真实负荷/吸收/理解/战斗/资源消化会让精确位置前进；为什么大多数人不能只靠狠狠干两个月就连续升级；最主要的伤停/恢复、资源、身体/伴生承受或突破瓶颈是什么；天赋、功法、资源、际遇分别改变哪一环”。不要求工程公式或逐级参数，但不能只写“长期训练后自然提高”。**精确力量主尺是强制 Root Grammar**：World 必须选择 `连续数字 / 大境界+数字子级 / 数字序列` 之一，并给出含 `{N}` 的精确位置格式、明确数字精度规则、当前可见数字范围和少量大档位；任何主要修炼者都应能被写成一个唯一精确位置。`主尺名称` 只是兼容字段，默认 `NONE`；当“某境7重 / 43级”本身已经清楚时，不再额外创造“某某尺/某某序”给整套等级二次命名。它是 Reader Ruler，不变成总战力分或机械胜负公式；但也不能只是装饰数字。World 的少量 benchmark 必须真实说明当前常用档位至少哪些**基础盘**会随主尺提高——例如输出/能量总量、身体承受、速度反应、作用范围、持续、可进入环境或题材自己的等价事实。后续相性克制只自动抹掉它真实克制的部分，不能因为高阶者一招失效就默认其余主尺基础盘同时消失。**Small Grammar, Large Variation**：已有一到少数互补力量操作轴如果已经简单、有辨识度，就保护它，不为“更统一”泛化成元能量/总机制；Small Grammar 不等于 Small World，主动让旧规则长出新招式/战斗姿态、身体/物种、兵器/奇物、异兽/伴生物、会改变玩法的环境、组合与稀有例外，让读者旧知识持续复利。**World Possibility Ecology 关注的是因果分布，不是能力条目数量**：高价值成长来源要自然挂在不同人物、地点、承诺、季节/时间窗口与长期路线，允许真实选择一条以后错过另一条；不要让一个最显眼机制成为所有人生都会回到的 universal upgrade trunk，也不做能力类别配额。World 同时主动寻找 0—1 条真正成熟的 Optional Secondary Fantasy Road；没有足够好的创意就不造，也不预设未来主角一定会走。创新放在力量因果和玩法，不靠多个新词互相解释；默认直接力量先写身体、攻击、防御、移动、元素、兵器等可感知效果。对当前会频繁使用的少数力量/价值/身份档位给 1—2 个可感知、可复用 benchmark，供下游稳定比较，不建战力数据库；
- **Public Milestone Ladder = Future Power Promise**：精确主尺告诉读者“现在多强”，少数大阶段则提前告诉读者“以后会第一次多出什么可能”。当前可见 Horizon 中真正的大节点必须是 `Major Stage = Body Change = Capability Milestone = World Aperture`：至少出现一个过去绝不可能的直接动作，并带来可观察的社会识别和新的环境/地图/身份/人物/资源入口；再给出低阶人物会具体羡慕它的理由。**“公共新动词”是后台字段，不是能力命名位：写“能稳定飞行 / 能把感知送到体外 / 能在无空气环境生存”即可，同时说明身体/精神为什么因此做得到，不再造“悬行/离察/镇轨”类二次短名。** 对身体成长型玄幻，普通飞行、恶劣环境承受、离星/横渡等基础行动权由境界本身解释；器物/阵法/身份只提供低阶借用、效率、合法准入或特殊路线，不能变成与境界平行的行动权限系统。大档名有意义时优先成为第一力量身份，精确数字用于第二层校准；连续小级仍可只增长基础盘，不逐级列能力、不固定 Milestone 数量。`Public Milestone ≠ Protagonist Expression`：World 只冻结公共版本，Power 才负责 Core 到同一节点时的个人表达；Story Program 借真实高阶人物/冲突/入口把重要未来节点提前演示成 Expectation Ladder，Outline 用现有 Reader Release 只排最少充分事实，不在开篇倾倒整张梯，也不为示范另造教学剧情；**当高阶者已经现场展示某个 Public Milestone 的公共能力，或主角当前明确做不到一件自己想做、而该 Milestone 正好能打开的事时，最早这个真实因果点就应释放 `大阶段名/精确门槛 + 直接能力 + 一项当前相关 World Aperture`，不等临近突破才命名。**
...
创新边界：**设定创新 ≠ 术语创新 ≠ 机制复杂化，Novelty ≠ Power Fantasy 强度**。Power Asymmetry 不强制来自世界内合法例外；它可以是稀有天赋/体质、唯一奇物/际遇、外来知识/经验、外挂、正常维度上的极端天赋或少量优势叠加。每个候选最多一个主异常，必须先用一句大白话讲清“别人做不到什么，我具体多能做什么”。Lexique primitive 只有在它让同一主异常获得更具体的身体/器物/空间载体或真正新玩法时才采用；只是换皮、增加限制、变玄或降低 Privilege Delta 时直接忽略，不得借它改写 Novelty Spark / POWER BASELINE 已有触发、覆盖、代价或 Boundary。命名同样**语义先于气味**：固定 Naming Craft Reference 只帮助读者理解与词根复利；首读准确高于世界气味，只有不牺牲准确时才复用 World 已有具体词，lexique 次之，也可以完全不用。普通短名已经准确、顺口、低学习成本时，不为“更独特”强行改写；名字多承诺了来源、对象、代价、等级或第二系统时，改名字，不扩机制迁就它。默认强度故意偏夸张：宁可初稿偏强一档，也不要让 LLM 把优势平衡成“更方便”；相对同层普通人/天才至少一个维度要明显超标，甚至提前拥有通常更高一大档或数档才有的**局部特权**。但 `局部高阶特权 ≠ 开局完整跨大档胜利`：Power Seed 必须保留“公共主尺决定哪些基础盘、Asymmetry 改变哪一部分”的真实耦合；一次新鲜相性克制可以让高阶者失手、受伤、丢物或让主角逃生，却不能自动清空对手其余主尺优势。**越级幅度本身属于长期成长**：早期先让同阶优势和小幅越级成立，后续随着主尺本身上升、异常掌握、新 Asymmetry 与复合能力积累，再自然扩大到多档越级；不为所有世界统一规定 `+N级`。Permanent Boundary 防万能，不做对称成本结算，优先收束成一到少数真正根边界，必须留下明显纯收益；长期默认 **Boundary Stable, Privilege Expands**，不随成长同步追加疲劳、反噬、冷却等平账条款。直接型能力的长期掌握继续扩大控制、对象、战斗复合和危险场景下的稳定使用，不变回结构分析、材料诊断、路线计算或验证流程；Legendary / Future Legend 也不能绕过 Permanent Boundary。为什么“馋”必须落到读者立刻想使用的具体体验；长期还应能与功法、装备、环境、传承等产生新化学反应，而不只增加数量与距离。若 Core 天然累积同类变体，优先维持一个 reader-facing 的个人能力语法，让新变体、旧变体升级与复合都挂在同一棵树上，并继续由公共主尺限制输出规模；不把长篇写成互不相关的技能背包，也不强迫非累积型小说使用槽位制。

#### 负责

- 一句话能理解、能想拥有的 Core Fantasy；
- 正常修炼轴：世界本来的成长怎样真实增强主角本人；
- **Growth Coupling**：读取 World 已冻结的 Power Growth Causality，明确普通人真正卡在哪里，以及这个 Asymmetry 是否改变学习/吸收/身体或伴生承受/伤停恢复/资源效率/实战反馈/突破条件/机会获取中的某一环。若改变，就写清“普通人卡在哪里 → 它改了哪一环 → 为什么因此可能更快、更稳或上限不同”；若没有真实关系，明确 `不改变正常修炼速度`，不能为了后续高速升级偷偷补加速功能；
...
- `Current Power Portfolio`：正文已经真实获得/证明的后续能力、身体变化、武器使用能力与 Advantage Stack，由 `PERSISTENT CANON → Power / Capability` 持续更新；该小节第一行固定保存 `Current Power Position｜主尺：…｜精确位置：…`。Human Seed 冻结 T0 精确位置；State 只有在最终正文明确突破时才更新，越级胜利/社会重新估价不能反推升级。累积同类变体型 Core 在这里继续用已成立的统一个人能力语法压缩保存；外部兵器/法宝/坐骑的所有权留在 Possession / Tracked Assets，不与人物自身力量身份混写。后续 State 若漏写该行，代码继承上一 Canon 位置，避免几百章里因一次抽取遗漏丢失主坐标。
...
- **Protagonist Asymmetry Dominance**：持续获得新的高价值 Asymmetry 并递归复合成越来越难复制的 Advantage Stack，默认是被作者选中的主角的长期叙事特权。NPC / Rival 仍可正常成长、阶段性更强、拥有主角没有的神兵/异兽/体质/绝技、拿走主角错失的 signature reward，甚至在某个专业长期第一；但不要为了让普通长期 Rival 永远同档，就在主角每次新增优势后也自动给它补一个新异常/奇物/复合，写成第二主角。只有极少数被 Story Program 明确设计成镜像宿敌、共同成长极或最终 Boss 的角色可成为例外。
...
- **Living Power Progression / Rapid Growth Needs Protagonist-Specific Causality**：力量成长不能只留下“起点数字 → 时间跳跃 → 终点数字”。Story Program 必须沿 World `Power Growth Causality` 与 Power `Growth Coupling` 判断已有战斗、训练、探索、受伤恢复、资源获得、功法学习或伙伴变化究竟怎样改变 Power State；可以是真实升级，也可以是逼近但未突破、当前发挥受伤势压低/恢复、瓶颈被看见、Asymmetry 掌握变强或 Growth Coupling 第一次被证明。没有真实成长因果就保持原位，不为“成长感”造小升级。已知 Milestone 成为明确期待后，利用本来已有的故事留下少量 `Distance Closing`：新的精确位置、以前做不到而现在能做到的中间动作、仍失败的最后门槛、成长速度比较或离门槛还差多少；不是每一级都写，也不另造验级章。若作者明确要求异常快升级，Core Asymmetry / Advantage Stack 必须有读者可观察的效率、恢复、资源、实战反馈、风险收益或机会获取因果解释速度；不能只由情节按章递送升级材料。
- **Core Growth Causality 必须 Reader-Explicit**：第一次进入主力量体系时，若前几章已有受伤恢复、共鸣/修炼磨合或其它自然机会，就直接用普通话建立一次“普通人怎样让位置前进 + 为什么不能无限快 + 主角 Asymmetry 是否改变其中一环”；之后采用 `Tell clearly → Show repeatedly → Tell the new delta`。静态能力边界会随 Mechanism Explanation Decay 衰减，但动态成长因果、当前精确位置、状态变化与距离下一质变不会因为“以前解释过能力”而一起消失。
...
- **Local Apex ≠ Final Apex**：普通非终局 Horizon 活透后仍按 `World Horizon Handoff → World Expansion` 前进，不要求主角离开每张地图前把所有专业/榜单刷成第一。若作者明确当前就是小说最终 Horizon、不会再 Expansion，则同一 Handoff 第一行改为 `FINAL NOVEL END`：主角进入最终公开力量最高可见圈层，并在最后决定性胜负/生存/世界选择中用长期 Advantage Stack 证明同档的决定性 Asymmetry Dominance；Rival 可同级、可活着、可保留某一专业第一。终局可保留没有解释尽的世界余白，但它们不再成为下一轮主线义务；“远期仍值得追”固定结束，不再用“山外有山”逃避结账。
...
**Longitudinal Cast 只给少数真正还在“过自己人生”的人物，不是 Important NPC List。** Story Program / Refresh 对这类人物除了保存“怎样影响主角”，还保留 `自己的未完人生 + 已启动行动`：如果主角暂时消失，他仍会因已经成立的目标、承诺、损失、限制或关系做什么。主角不是所有关系的唯一中心；两个重要人物之间已经存在、且会改变行动的亲缘、爱情、师徒、竞争、利益、债、背叛、效忠或共同历史，可以继续成为因果，但没有自然关系时不补线、不画全员关系图。离屏演化只能从已发生事实 forward，不能在重新登场时 retrospective backfill 一段关键奇遇或重大获得。成熟人物线的大高潮遵守 **Convergence ≠ Recall**：必须由共享的人、物、地点、身份、时间窗口或决定形成因果铰链，而不是把旧人物统一召回站队。讲完的人允许结束；TGN 不因此新增全员 Human Seed、Character Agent 或回访配额，主角仍独占递归 Advantage Stack 的长期中心性。
...
- 根据已批准长期方向、当前剧情块、Future 10 和 Canon，生成当前章事件合同；Future 10 当前条目在进入 Director 前先被确定性拆成 `本章唯一可执行事件预算` 与 `章末 Handoff Reservation`，Long Block 只作阶段背景；带明确章节范围的 Long Block 只有覆盖当前章才进入，显式过期或无合法匹配时 fail closed，不回退整份旧长纲；后者只能制造下一章为什么必须发生，不能提前付款、正式身份、获得、升级或其它下一章结算；反向也保护即时连续性：上一章若停在堵路/拔刀、追杀未脱身、攻击已落下、坠落/被困、关闭中的门或必须当场回应的交易/选择，Director 先用最低充分动作接续、结束或转化该 Canon 局面，再进入当前章 Plan，不能直接跳到次日、另一地点或“已经进入”后的状态；桥接必须包含一个可落正文的具体动作因果，不能只写“趁乱脱身 / 成功进入 / 摆脱追兵”；桥接本身不授权新增重大胜负、奖励、关系翻转或资源结果；
...
**经典原文评审使用 `novel-original-text-review`，不使用 GBrain 冒充 source reading。** 该 Skill 从本地完整原著库动态发现书目、解析章节、搜索同位场景并生成带 source path / chapter / line range 的连续阅读窗口。用户未点名时可以从当前数百本完整原著中选 1—3 本最合适的作品；路由建议只是 search prior，不是固定书单。只有 Agent 实际打开并读过原文窗口后，才能输出 `Original Text Reading Receipt` 并声称完成经典原文对照。该能力只服务开发审计 / Scene Craft 研究，不进入 production Prompt。
