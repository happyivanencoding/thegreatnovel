...
- 返回给规划 LLM 的 `可用抽象` 来自原卡中文写作区块，英文 aliases 不进入 Prompt。`book-dna / arcs` 在原有 800 字预算内优先当前阶段对应的 `Guidance`，再用余量补 Reader Promise / Mechanism 等背景，避免长原作摘要挤掉可迁移方法；没有可匹配 Guidance 的卡沿用既有区块顺序。规划参考不能据此变成 Canon；章节层仍只接收已批准故事与 source-blind craft。
