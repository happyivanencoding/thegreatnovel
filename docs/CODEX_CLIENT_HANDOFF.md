...
`proposal_schema.json`，调用 `$bootstrap-original-novel`，只允许写
`artifacts/story_foundation/proposal.json`。结果必须恰好包含三个 Story Foundation
candidate id，且保持 `PROPOSAL`；导入结果和作者显式“确认基础框架”是两个独立动作。
