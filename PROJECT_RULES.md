...
- **Longitudinal Cast ≠ Important NPC List；人物关系不以主角为唯一中心**：只有少数已经拥有自主欲望、未完人生与已启动因果的人物需要跨 Horizon 继续；如果主角暂时消失，他们仍会因已有目标、承诺、损失、限制或关系向前行动。人物—人物之间已成立且会改变行动的亲缘、爱情、师徒、竞争、利益、债、背叛、效忠或共同历史可以进入长期因果，主角只是其中一条关系边；没有真实因果不画全员关系网。离屏推进只能 forward from established facts，不能 retrospective backfill 关键人生/重大获得；回流优先带来离场后才会出现的新状态。多线高潮遵守 **Convergence ≠ Recall**：只有共享因果铰链让各自正在行动的成熟线自然碰撞，不能把旧人物机械叫回来站队。讲完的人允许结束；不新增全员 Human Seed / Character Agent / thread database / 回访配额，普通配角也不因“丰富”复制主角递归 Advantage Stack。
