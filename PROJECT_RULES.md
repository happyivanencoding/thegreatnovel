...
- **Approved World Vision 是章节期一等世界事实权威**：BOOK §2 只是故事摘要，不能替代 World。Runtime 将 World Vision 确定性裁成不含 named 大事件 / 未解谜底的 `WORLD AUTHORITY`；Outline 是 Reader Release Scheduler，把当前要释放的具体 World fact 写进现有 `具体剧情`；Curator 只从 WORLD AUTHORITY 做 bounded prefetch，Writer 只表达、不自行选择或补造世界事实。高价值 Orientation 可以用短直接旁白，但不做百科、Prelude 或每章说明配额。
