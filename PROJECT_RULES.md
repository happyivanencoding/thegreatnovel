...
- Author Workspace 的 `agentdock_acp` 是浏览器内唯一 production ACP 执行面：后端只从可信配置解析入口，固定 TGN project root、`mcpServers=[]` 与 `read-only`；`fs/read_text_file` 只读取 project root 内 UTF-8 文件；Codex 的 `execute` permission 只允许单次并继续受 read-only sandbox 约束，`edit` / file-write / permission escalation 继续拒绝。浏览器不得提交 executable、cwd、mode 或 MCP，status 不得泄露本机路径/用户名。
...
长期 Mystery 允许使用低频 **Progressive Canonization / 渐进定真**：`AUTHOR OPEN → Mystery Decision Surface → DEFER / DECISION NEEDED → Non-Canon Reframe R1/R2/R3/D0 → TGN operator 选择 → Independent Mystery Compiler → AUTHOR FIXED HIDDEN → Story/World Planning → reader-facing Reveal Event → State/Canon → 更深 AUTHOR OPEN`。触发条件不是章节数、伏笔年龄或“大纲应该完整”，而是**下一段已冻结的具体故事需求已经无法在不决定某一小层答案的情况下成立**；还能继续写好故事就必须允许 `DEFER`。自动 Production Run 中该最小决定由 operator 在用户方向与既有 Authority 内完成，不中断用户阅读流程。

- **没有 production Fantasy Seed。** Premise Aperture 是可丢弃的 Non-Canon 搜索与编译阶段，不是第四 Authority。
- Premise 从未开始或 operator 选择跳过时，原 Split Authority 路径保持可用；一旦保存候选，World / Power / Human / Story 的生成与 Freeze 必须等待 strict `PASS` + operator 采用，不能静默绕过。Compiler Input 必须在 Prompt 生成当下落盘，报告保存不能重绑到后来编辑的文本；snapshot 与当前 selected card 不一致时必须重新编译。World Vision 一旦 Freeze，Premise 决定冻结。
- Forge 一次生成 S1/S2/S3，不由 Forge 自己排名或自选；Independent Compiler 只审 trigger、载体、T0 尺位、Interface 因果与远期复合，不评分、不改稿、不偏向保守。`CONDITIONAL PASS / FAIL` 返回 TGN operator 处理；没有额外常驻 selector 或 Repair Loop。自动 Production Run 的 operator 选择属于任务编排，不是 Forge 自评。
- operator 采用后由代码确定性拆成 World / Power / Human / Story 四条 lane contract；Story Program 第一次读取完整 Promise，Outline 与章节 Runtime 不再读取 raw Premise Card。无法同时成立时必须 fail loud `PREMISE-AUTHORITY CONFLICT` 并由原 runner 在既有方向内处理最早失败层。
- **没有 Character Composer LLM。** Power/Human 不做后验主题化调和。
- Freeze checkpoint 保持紧凑：（可选）Premise → World Vision → Character（Power + Human 一次 Freeze）→ Story Program。它们是 Agent 间信息边界，不是默认用户交互步骤。
- Forward Authority 仍需被 operator 显式采用：World Expansion 候选只有采用后生效；Human Development 可返回 `NONE`，只有采用的真实 Delta 才进入长期人物权威；Story Refresh 复用 Story Program 的现有保存/Freeze 流程。**模型生成不自动成为 Authority**，但“需要显式采用”不等于“需要用户点击”。
- **作者未知本身是合法状态**：`AUTHOR OPEN` 表示当前尚未冻结答案，不是待模型补齐的缺陷；`AUTHOR FIXED HIDDEN` 才表示答案已冻结、但读者/人物尚未知。Decision Surface 不给答案；Reframe 不自动选择；Compiler 只允许回答当次 `Smallest Decision`，候选自己的 `What Remains Unknown` 成为新的保护边界。Compiler FAIL 返回 TGN operator 处理，没有自动 Repair Loop；自动 Production Run 不因此要求用户中途介入。
...
- **Relationship-History Backfill 是 Story Program / Story Refresh 的合法创作权，不再限于“少量非奠基性过去”**：作者明确允许在 World / Human / Canon 尚未定义的空白里补重要配角旧史、隐藏关系、旧爱/亲缘/师徒/竞争、共同失败、救命、背叛、债、失约、失去的人与上一代选择，并允许把 Human 已成立的父母/兄妹/旧关系与 World Living Actors 接成过去因果。优先能自然穿过多个既有人物、甚至跨代留下后果的共享历史，而不是给多个配角各写互不相干的“深刻往事”；过去选择应留下今天可见的身份、误解、债、遗物、空位或路线，并让至少两个人因此采取不同当前行动。**过去空白可以补，已经发生/公开的 Canon 不能改，AUTHOR OPEN 不能偷答，离屏未来也不能事后伪造成已发生奇遇/神器/联盟。** 作者批准后的 Story Program / Refresh 才把新增旧史变成 Authority；若完整真相适合以后揭露，当前只保留表层关系、可见残留和会在揭露后改变现实路径的隐藏层，不一次性倾倒。不得自动悲情化，也不得用过去证明整个人。
...
- **Canon Retained ≠ Plot Advancing；自然 Horizon 使用 Local Closure + Book State Mutation 双结算**：Rival、旧人物、旧资产、家族/故乡线、长期 Mystery 或具名旧债被完整留在 `PERSISTENT CANON / OPEN PROMISES / Handoff` 只说明系统没有失忆；长期线可以完整休眠一个甚至多个 Horizon。真正推进必须由新物证、新策略、新关系位置、新价格、新限制或新行动窗口改变后续选择。Horizon 结束时 Local Closure 负责把当前世界主要故事真正讲完；Book State Mutation 负责让至少一个以后仍与整本书相关的人、关系、身份、资产、敌人策略、价格、知识或世界事实从此不能完全按旧状态运作，并产生新的行动条件。这个主体可以来自成熟旧线，也可以是当前 Horizon 新建立的长期因果；**没有“每个 Horizon 必须推进 1—3 条旧线”的回访税**。多世界 instance 若真实返回主世界/Hub，原有更强的 `Main-World Return Consequence` 继续成立：回归不能退化成“确认能力 → 看下一门 → 再进副本”；旧获得应自然改变力量、身份、敌人、关系、价格、入口或 Mystery。没有传播/接触因果时不强插消息或召回。
- **Book-Level Longitudinal Spine 只固定跨 Horizon 的问题，不预写未来世界**：多世界长篇应在 `全书成长与核心幻想兑现脊柱` 中保有约 2—4 条能支撑 60—120 章的长线，例如长期 Rival、主角人生/家族线、Meta Mystery、主世界社会位置或可反复回访的重要关系/资产。每条只保存 `当前仍在问什么 → 下一次什么类型的新事实会改变它 → 当前仍必须未知什么`；未来具体 World、人物、奖励、能力、章号和 Mystery 真相继续由后续 protagonist-blind World Expansion + fresh Re-Collision 决定。Spine 可以休眠，Story Program 不为“长篇感”机械召回；Spine 是问题压力，不是 thread database、回访配额或提前写死的百章大纲。
- **Longitudinal Cast ≠ Important NPC List；人物关系不以主角为唯一中心；Character History 不只是当前关系边**：只有少数拥有自主欲望、未完人生与已启动因果的人物需要跨 Horizon 继续；如果主角暂时消失，他们仍会因已有目标、承诺、损失、限制或关系向前行动。人物—人物之间的亲缘、爱情、师徒、竞争、利益、债、背叛、效忠或共同历史可以先由 World protagonist-blind 地成立，也可以由 Story Program / Refresh 在未定义空白中合法 backfill；高价值旧史优先形成 `Past Choice → Present Residue → Independent Current Actions → Delayed Reinterpretation → Convergence / Second Payoff`，自然时允许上一代选择继续改变下一代身份、误解、债、遗物或路线。**过去可补，未来不能假装已经发生**：从当前时点往后的离屏推进仍只能 forward from 已批准事实，不能回流时凭空补神器、联盟或重大成长。主角只是关系网中的一条边；没有真实因果不画全员关系网。多线高潮遵守 **Convergence ≠ Recall**：只有共享因果铰链让各自正在行动的成熟线自然碰撞，不能把旧人物机械叫回来站队。讲完的人允许结束；不新增全员 Human Seed / Character Agent / thread database / 回访配额，普通配角也不因“丰富”复制主角递归 Advantage Stack。
- **Historical Recontextualization 是纵向复利的一部分**：长期复利不只是“旧能力/资产还在”。Power / Asset / Relationship / Identity / Knowledge 进入新的敌人、关系或世界条件后，应有机会改变用途、选择价值、风险或社会价格；高价值链可概括为 `Carry → Recontextualize → Combine → Consequence / Reprice`。若只是换地图后把旧能力原样再用一次，不算新的 compounding；也不要求每个 Horizon 给每件旧资产强行换功能。
- **Opportunity Collision ≠ Task Board**：多个高价值路线可以并存、交易与报酬可以明确，但优先让机会作为 Living Actors 正在做的事，在同一个人/物/地点/时间窗口上互相碰撞；避免稳定生成“三个 NPC 依次报任务+奖励 → 主角比较菜单 → 选择一个”。若删掉任务摘要后各条线不能在没有主角时继续发生，最早问题在 Story Program / Outline 的碰撞结构。
- **Local Ruler 可以换，长篇必须有不重置的 Progress Ruler**：默认先使用 World Root 的公开主力量尺；若 Plot Pace 明显快于 Tier Pace，多世界长篇可以从已经发生的故事中再建立一条简单 Meta 进度尺，说明核心跨界器物/界门能力目前稳定能做哪一种可观察操作。它不是总战力分，不固定某套阶段，不预授权未来能力；Local ruler 负责当前世界坐标，Global ruler 负责几十章累计距离。
- **两层 Power**：Frozen Power Origin + `PERSISTENT CANON → Power / Capability` 的 Current Power Portfolio；后期神兵/传承/新 Asymmetry 不回写 Seed。若 Core 天然累积同类变体，Current Power Portfolio 应继续按同一个 reader-facing 个人能力语法压缩保存“已有成员 / 已升级成员 / 已成立复合”，不要退化成互不相关的技能背包；外部兵器、法宝、坐骑的**所有权**继续属于 Possession / Tracked Assets，只有已经学会的使用方式或与自身力量形成的复合能力进入 Power / Capability。非累积型 Core 不强造槽位或第二公开体系。**三层 Human**：Frozen Origin + Current State + 可选 Human Development Delta。
- **精确力量主尺强制存在，并与 Public Proof 三线共用；Reader Ruler ≠ Battle Formula ≠ Decorative Number**：每个 production World Root 必须冻结能写出唯一当前位置的公开精确 Grammar，只允许 `连续数字`、`大境界+数字子级`、`数字序列` 三种简单形式；**精确不等于再给整套尺造专名**，当“化龙7重 / 43级 / 序列6”本身足够清楚时 `主尺名称=NONE`；“潮阶 / 身天尺 / 九门尺”这类只是在精确位置外再套一层标签的名字默认不要，只有社会本来就反复说这个短称且它能减少解释时才命名。Human T0 固定一个精确数字位置，State 在 `Power / Capability` 持续维护 `Current Power Position`。macro World Expansion 只能向上延展同一 Grammar 的可见范围，不得改计数语法；独立 instance 必须有自己的本地精确尺，但不回写全局主尺。精确位置不是机械胜负公式，也不建总战力分；技能、装备、经验、环境与 Power Asymmetry 仍可造成越级。**但公开主尺不能只是装饰数字**：World 对当前常用档位的少量 benchmark 必须真实承载一组不会被任一单项特殊机制凭空抹掉的基础盘，例如输出/能量总量、身体承受、速度反应、作用范围、持续、环境进入能力或题材自己的等价事实。相性克制只自动拿走它实际克制的那一部分；不能从“高阶者一招/一种感知失效”直接跳成“其余主尺优势也全部失效”。重大 Public Proof 若与主力量相关，**群体震动 + 懂行者 Ruler Calibration + 关键人物 Behavioral Repricing**三条线共同使用精确位置：读者应知道主角/对手到底几级、几星、几重、差多少，正常差距意味着什么，以及这个超标怎样改变现场和社会价格；越级胜利不得反推成等级自动提升。
- **Public Milestone Ladder = Future Power Promise；Major Stage = Body Change = Capability Milestone = World Aperture**：精确主尺负责让读者持续知道“现在多强”，World Vision 还必须在当前可见范围内冻结少数真正的大阶段公共质变。**对人体成长型玄幻，飞行、离体感知、耐寒/耐毒/缺氧、长期御空、星空生存/横渡、影响山河等基础行动权优先由境界自身的身体/精神结构变化解释**，让“人变成了什么”直接回答“世界少了哪层限制”；高境界新能力必须从前面已经建立的身体/精神 Grammar 继续长出，例如内部循环更完整所以能离开古星、自身运行足够稳定所以能把已经掌握的力量方式扩到更大外界范围，不用“到王境便获得任意法则权限”一类抽象万能跳步；器物/阵法/异兽可以让低阶者借用、加速或走特殊路线，身份可以限制合法准入，但不能另造一套与境界平行的基础行动权限。每个 Milestone 至少有一个过去绝不可能、到此通常第一次能做的直接动作，以及一句“身体/精神为什么做到”、可观察社会识别、由此打开的环境/地图/人物/资源入口、低阶人物为什么会具体向往它。**`公共新动词` 只是后台字段名，不给能力再起“悬行/离察/镇轨/应星”一类短名；直接写“能稳定飞行 / 能把感知送到体外 / 能在无空气环境长期生存”。** 大档名存在时优先作为第一力量身份，精确数字作为第二层校准；连续小级可以只增长基础盘，不逐级填能力，也不规定固定 Milestone 数量。`Public Milestone ≠ Protagonist Expression`：World 冻结所有正常修炼者共享的质变；Power 只决定 Core 到该公共节点时怎样形成更独特的个人表达；Story Program 负责把值得追的未来节点提前变成 Expectation Ladder，优先借本来就在行动的高阶人物/冲突/世界入口**先演示后抵达**，让 Frozen Human 因自己的欲望真实向往/竞争/畏惧。Outline 只用现有 Reader Release 排程最少充分的公共事实，不在开篇倾倒整张 Ladder，也不为展示里程碑新增教学/复测/示范章。未来 Milestone 的 Reader Release 触发点取最早真实因果；macro World Expansion 只为新公开范围增加新的 Milestone，不重写 Root；独立 instance 可有自己的 Local Ladder。不要复制魂环槽位，不建每境界技能表或第二战力系统。
- **World 力量优先 Small Grammar, Large Variation；复杂度放进历史、关系与尺度，不放进词汇表。** 如果主流力量已经能用 1—3 句普通话讲清，且现有一到少数互补操作轴已经有辨识度，就保护它，不为了“统一”泛化成元能量/总机制。Small Grammar 不等于 Small World：后续把 Variation 预算花在新招式/战斗姿态、身体/物种、兵器/奇物、异兽/伴生物、已有 Grammar 在不同环境中的组合、强度与少数稀有例外；**概念预算优先给真正值得记住的奇观、人物、势力与高价值遗物**，普通药材/材料/交通/凭证/测量工具默认用普通类别名。当前 Horizon 可以有很多地点，但只有少数 signature 奇观需要独立异常机制；更多地点应靠普通地理、谁住在那里、谁曾在那里赢/死/留下传承、今天谁正在争什么而变深，读者每到一地不应像重学一个小系统。只有旧语法承载不了一个长期值得追的新幻想时才新增底层机制。**Opening World 与每次 World Expansion 统一采用 Progressive World Skeleton / Think Deeply, Freeze Sparsely**：只把 World Root、当前自然 Horizon、当前 Living Actors、今天仍在产生后果的 Causal History 与少量 Adaptive Scale Promise 冻结成 Authority；未来局部城市、副本、支线遗迹、市场/分舵、一次性资源、尚未进入当前因果的人物与旧史布景默认 OPEN。**细节按因果需求加载，不按世界规模加载**：一旦继续增加事实已不会改变当前人物可行动选择、空间因果、价值判断、Reader orientation 或未来一致性，就停止具体化；未写出的内容默认 OPEN，不建大串 TBD。新 Horizon 通常收在少量可复用空间/势力锚点和少量无主角也会推进的当前事件，并至少让一条主要欲望直接指向本 Horizon 的奇观、强者、生物、传承、兵器、身体变化或其它 Fantasy Surface；修路、护运、维修、配给、路线管理等只能作 supporting logic，不能占满新世界发动机。Scale Promise 只冻结“世界怎样变大 + 当前必须稳定的最少根事实”，Landmark 由本书题材自己长出，不套古墓/宗门/城市/星路模板，也不是主角必去路线；Final Scale 可稳定，但 Final Boss、最终传承、最终战场与终极谜底不因 Scale Promise 自动冻结。Scale Promise / Deferred World 属于 planning/backstage Authority，不进入章节 Writer 的安全 World Reality。**World Possibility Ecology 是因果分布，不是能力数量**：高价值“变强/变得不一样”的来源应自然分散在不同 Living Actors、地点、承诺、季节/时间窗口与长期路线，结果可以落到不同身体、器物、传承、伙伴/异兽、环境特权或知识；不要让一个最显眼机制成为所有路线都会经过的 universal upgrade trunk，也不按类别配额凑能力。World 同时主动寻找 0—1 条真正成熟的 Optional Secondary Fantasy Road；没有足够好的创意就不造，也不预设主角一定会走。
- **World Independence = Living Actors + Existing Human History，不是机构在运转**：`世界正在发生的大事` 优先由具体人物、生物或小群体的私人目标与下一步可见动作发动；机构、战争、市场和资源格局负责放大后果，不默认替代人物成为发动机。至少若干事件应能回答“谁现在想要什么 → 马上准备做什么 → 没有主角也会改变谁/哪件东西/哪个地方”。同时 World Vision / World Expansion 允许少数 Living Actors 在未来主角出现以前就彼此活过真实历史：旧爱、亲缘、师徒、竞争、背叛、救命、债、共同失败、失约、继承、上一代选择等只要会改变今天行动即可。高价值关系结优先保留“一个过去选择同时压住第三个人/下一代”的传导，并允许当前存在可信表层解释、互相矛盾的记忆、遗物/空位与更深尚未 reader-facing 的旧因果；不把所有旧史压成 A↔B 双人恩怨。多线自然碰撞优先来自争同一具体人/物/地点、抢先抵达、带走/毁掉某物或互相追杀；不新增 Actor schema / Event table / 全员关系网 / 代际配额。
- **Power Seed 生成 Power Asymmetry，不强制世界内合法例外**：World Power Normal 是比较尺，不是来源限制；非对称优势可来自世界内稀有异常、唯一奇物/际遇、外来知识/经验、外挂、极端正常天赋或少量优势叠加。默认故意偏强：Core Power 至少一个维度让同层普通人/天才明显羡慕，必要时提前拥有通常更高层才有的**局部特权**；但 `局部高阶特权 ≠ 开局完整跨大档胜利`。Power 必须保留与公共主尺的真实耦合：主尺继续决定 World 已批准的基础盘与当前可承载/输出尺度，Asymmetry 说明自己具体改变哪一部分。开局一次新鲜克制可以让远高阶失手、受伤、丢物、被迫重整或让主角逃生；跨明显大档的完整击败需要已经成立的多重优势因果共同支撑。**越级幅度本身是长期成长资产**：早期先同阶突出/小幅越级，随着主尺上涨、异常掌握、新 Asymmetry、Advantage Stack 与复合能力真实积累，再允许完整越级越来越夸张；不跨书硬编码统一 `+N级`。Permanent Boundary 防万能但不做对称成本抵消。Novelty Spark 负责不同，不负责削弱强度。Power 另接一小池 **Optional Lexique Primitive Spark**：对象×变化只负责偶尔找到更具体的身体/器物/空间载体或新玩法，每个 Candidate 最多借 0—1 个且可以全部忽略，不得改写既有触发、覆盖、代价、Boundary，也不得长成第二系统。**Reader-facing novelty = 熟悉语言 + 新作用**：World 基础力量、开局 Core 与后续新 Asymmetry 都先用普通话说明具体可观察效果，再决定是否需要短名；命名固定参考来自 source-blind Naming Craft；**首读语义准确高于世界气味**，只有不牺牲准确时才优先复用 World 已有具体词根，lexique 只作次级气味，也可以不用；已有普通短名已经准确时不为“更独特”强改，名字不能反向授权新机制。“全新”改变力量因果/玩法，不要求回避境界、功法、兵器、异兽等清楚题材词；若核心幻想本来是战斗、身体、移动、穿越、操控等直接能力，成长不能重新退化成结构分析、材料诊断、路线计算或验证流程。Legendary / Future Legend 不得绕过 Permanent Boundary。
...
- **世界前台尺是长期读者坐标，也是信息压缩器；Living Power Ecology**：力量、战绩、价值、天赋/适配、技熟、装备、排名、身份与世界层级都可作尺；当前事件碰到哪把就校准哪把。World 对当前常用少数档位/价值对象提供 1—2 个可感知 benchmark；下游优先用一次已批准比较替代多轮验证，不临时发明冲突标准，也不建战力数据库。**本章若真实跨过前文已说明的公开力量/身份档位，结果处直接命名新档位一次**，不让读者从“凝影了 / 被记名 / 通过了”等现象自己换算。重要 World Entry / Rival / 传承或高价值机会第一次进入时，同时让读者知道为什么值得靠近或争、它相对主角当前位置有多重；若后续要放弃某个机会，先建立其具体价值，否则机会成本只存在于计划里。**这把尺不只属于主角/Rival**：重要人物已有公开精确位置且它解释他为何能站在这里、别人为何让路/忌惮、他能守哪条线或为何有能力却选择不介入时，同样在自然动作中进入 Reader model；没有 Authority 不补数字，不做全员等级表。
- **Story-approved Actor Ruler 绑定不得在 Outline 压缩时丢失或重调度**：Story Program 已经明确决定的少量 `具名人物 + 精确位置 + 当前 Horizon 的具体阶段/地点 + 公共能力/里程碑展示 + 现场意义`，保存到 `Reader-Facing Actor Ruler Anchors`（通常 `NONE`、最多0—6条）；Outline 在对应 Schedule 原样运输，不能换演员/数字/展示/地点或搬到更晚阶段，Future 10 到达边界时必须兑现。它不是新 Authority、等级表、RSE 或新 Agent，也不允许为了填表临时给人物发等级/安排展示；只防止已经决定的 Story causality 在自由文本编译中丢绑定。
...
- **Reader-Facing Scene Ecology：复杂场景先给读者一个能长期拿住的舞台与人物模型**。连续跨章的多方 Action Block 在第一处真正复杂动作前先确立 3—5 个 Stable Scene Geography 锚点及关系，后续复用同一组名词；新行动点先说明相对旧锚点的位置，不为每个动作重新发明舞台。`Action Advance ≠ Situation Memory`：动作推进两三轮、人物换位、通路开闭或目标变化后，允许用1—4句直接重报谁在哪、哪条路开着、谁守什么、还差哪个点、主角现在要去哪。有限 POV 可以通过 `观察 → 判断/猜测 → 必要时发现判断不全 → 决定` 帮读者理解多方局势。重要配角当前真正起作用的 reader-safe 私人压力、旧关系或不愿承认的判断至少影响一次对白/停顿/注意力/拒绝/让步/选择；不倾倒 Biography，也不把所有人物压成一句任务。多线大碰撞遵守 `Convergence is payoff, not simultaneous loading`：大多数重要参与者先有最低充分的“是谁/要什么/为何会来/若已有且相关则在哪个力量层”锚点，可以在同一剧情块错峰建立，不要求额外介绍章。对白长短随人物和现场压力变化，不统一写成一句功能指令。
- **Mechanism Explanation Decay**：Authority 可以完整保存能力/规则边界，正文说明不能因此每次重念。第一次使用讲清最短普通话效果；第二次只补新的边界/失败/组合；第三次以后若动作足够让读者认出机制，优先动作 → 反应 → 结果。新复合玩法第一次出现时，尽量让读者先看到结果并自己意识到“还能这样”，只有理解风险真实存在时再补一句最短确认；章名也少用后台机制公式提前总结答案。
