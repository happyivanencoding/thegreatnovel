...
- **精确力量主尺强制存在，并与 Public Proof 三线共用；Reader Ruler ≠ Battle Formula ≠ Decorative Number**：每个 production World Root 必须冻结一把能写出唯一当前位置的公开主尺，只允许 `连续数字`、`大境界+数字子级`、`数字序列` 三种简单 Grammar；Human T0 固定一个精确数字位置，State 在 `Power / Capability` 持续维护 `Current Power Position`。macro World Expansion 只能向上延展同一主尺可见范围，不得改计数语法；独立 instance 必须有自己的本地精确尺，但不回写全局主尺。精确尺不是机械胜负公式，也不建总战力分；技能、装备、经验、环境与 Power Asymmetry 仍可造成越级。**但公开主尺不能只是装饰数字**：World 对当前常用档位的少量 benchmark 必须真实承载一组不会被任一单项特殊机制凭空抹掉的基础盘，例如输出/能量总量、身体承受、速度反应、作用范围、持续、环境进入能力或题材自己的等价事实。相性克制只自动拿走它实际克制的那一部分；不能从“高阶者一招/一种感知失效”直接跳成“其余主尺优势也全部失效”。重大 Public Proof 若与主力量相关，**群体震动 + 懂行者 Ruler Calibration + 关键人物 Behavioral Repricing**三条线共同使用精确位置：读者应知道主角/对手到底几级、几星、几重、差多少，正常差距意味着什么，以及这个超标怎样改变现场和社会价格；越级胜利不得反推成等级自动提升。
- **Public Milestone Ladder = Future Power Promise；Major Stage = Capability Milestone = World Aperture**：精确主尺负责让读者持续知道“现在多强”，World Vision 还必须在当前可见范围内冻结少数真正的大阶段公共质变：每个 Milestone 至少有一个过去绝不可能、到此通常第一次能做的**新动词**，以及能被世界识别的社会身份变化、由此打开的环境/地图/人物/资源入口、低阶人物为什么会具体向往它。大档名存在时优先作为第一力量身份，精确数字作为第二层校准（例如“铸脊，炉级41”，不是只剩“炉级41”）；连续小级可以只增长基础盘，不逐级填能力，也不规定固定 Milestone 数量。`Public Milestone ≠ Protagonist Expression`：World 冻结所有正常修炼者共享的质变；Power 只决定 Core 到该公共节点时怎样形成更独特的个人表达；Story Program 负责把值得追的未来节点提前变成 Expectation Ladder，优先借本来就在行动的高阶人物/冲突/世界入口**先演示后抵达**，让 Frozen Human 因自己的欲望真实向往/竞争/畏惧，最终突破时同时兑现公共新动词与主角版本。Outline 只用现有 Reader Release 排程最少充分的公共事实，不在开篇倾倒整张 Ladder，也不为展示里程碑新增教学/复测/示范章。**未来 Milestone 的 Reader Release 触发点取最早真实因果：高阶者已经现场展示该公共新动词，或主角当前明确“做不到某件想做的事”而该 Milestone 正好能打开它时，就释放 `大阶段名/精确门槛 + 公共新动词 + 当前相关的一项世界/社会开口`；不得默认拖到主角临近突破或主动喊升级目标时才命名。**macro World Expansion 只为新公开范围增加新的 Milestone，不重写 Root；独立 instance 可有自己的 Local Ladder。不要复制魂环槽位，不建每境界技能表或第二战力系统。
