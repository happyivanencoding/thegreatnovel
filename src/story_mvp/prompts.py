...
FANTASY_COMPOUNDING_DIRECTION = """幻想复利优先于操作流程复利：长篇中的 Power Asymmetry 应形成**持续积累的优势栈**，而不是每轮结束重新归零，也不能只让开局那一个能力无限放大。纵向成长可以同时发生三种变化：已有非对称优势继续深化；通过真实故事获得新的非对称优势；旧优势与新优势发生复合，产生任一单项都做不到的新玩法。新的非对称优势可以来自功法、技艺、身体/血脉、奇物/装备、知识经验、伴生体或本书自己的力量来源，只要它真正形成新的 Privilege Delta，而不是小便利、换皮技能或库存加一。
...
PROTAGONIST_BEHAVIOR_SIGNATURE_DIRECTION = """主角行为签名 / Decision Vector：主角“想要什么”之外，还要知道“他通常会怎样去要”。稳定的是跨场景的选择偏向，不是固定动作、口癖或每卷重复同一种题。**Signature ≠ Tension**：行为签名只回答偏转方向；真正的人物张力来自当前至少两样 Frozen Human 已经认可、都真有价值却不能同时完整取得的东西。Story Program 遇到这种自然冲突时，应让选择真实改变对象、顺序、路线、暴露程度、谁承担代价或哪项机会被放弃；允许人物找第三条路，但第三条路不能无损全拿。不要为了制造张力反向补童年/创伤，也不要把“安全 vs 自主”“钱 vs 人”等任何一组冲突固化成所有 Horizon 的人物模板。"""
...
**Behavior Signature 最好有生活根，但禁止反向造创伤。** 如果 Human Seed 已经独立建立过一个具体生活事实、旧物、旧关系、长期羞辱/失去/占有经验、家庭位置或反复发生的小现实，而且它自然解释“为什么这个人会特别在意某种选择权/所有权/输赢/靠近/自由”，可以把二者连成一条低频长期人物线，使读者以后回看时知道这个偏爱从哪里长出来。若现有生活事实没有这种支撑，就保持“尚不知道为什么”，不要为了让人物显得深刻临时补父母惨死、家产被夺、被背叛等悲情答案。**生活根负责加重量，不负责证明整个人格；它以后还必须能被新关系和新经历改变。**"""
...
LONGITUDINAL_THREAD_ADVANCE_DIRECTION = """Longitudinal Story Engine / 跨 Horizon 长篇推进（Story Program / Story Refresh / Outline 软原则，不新增 Agent、线程数据库或每卷回访税）：**Canon 被保存不等于故事在推进。** 对已经形成读者牵引的长期 Rival、人物关系、主世界身份、旧收益、家族/故乡线、长期 Mystery 或具名旧债，只有新物证、新策略、新关系位置、新价格、新限制或新行动窗口真实改变后续选择，才算 Plot advancing。长期线可以完整休眠一个甚至多个 Horizon；只在 OPEN PROMISES / Handoff 里列名、只写“某人仍在成长”或提醒一次旧物存在，都只是 retained / reminder，不是推进。
...
MAIN_WORLD_RETURN_CONSEQUENCE_DIRECTION = """Main-World Return Consequence / 回归必须反哺长篇（多世界长篇软原则，不新增固定章数税）：多世界 instance 结束后，如果人物真实回到主世界/Hub，回归不能长期退化为“确认能力还在 → 看下一扇门 → 再进副本”。上轮获得的 Power / Asset / Relationship / Identity / Knowledge 至少有一项，应在**本来就值得发生的主世界事件**里改变价格、身份、敌人策略、关系、机会或新的进入层级。重大新 Asymmetry 尤其优先在主世界已有 Rival、旧 ruler、旧社会估值或旧限制上制造一次 reader-facing consequence：过去挡路的人现在必须换战术，过去进不去的圈层主动开门，旧报价被推翻，旧家族/敌人重新判断，或新的高层人物因此找上来。
...
PERSISTENT_READER_RULER_DIRECTION = """Persistent Reader Ruler（长期读者坐标软原则，不新增战力数据库）：World Root 的**精确力量主尺**是主角力量成长的固定主坐标；主角在任何时点都应存在一个唯一 `Current Power Position`（几级 / 几星 / 几重 / 哪个数字序列），World Expansion 只延展范围，不重造主尺 Grammar。战绩、价值、天赋/适配、技法熟练度、装备品阶、排名、身份与世界层级仍可作为其它 ruler，但不能把精确主尺重新压成“大致低/中/高档”，也不合成单一总战力分。
...
**Living Power Ecology：重要人物也必须活在同一把公开力量尺上。** 如果 Approved Story / World 已经给某个当前窗口的重要人物明确精确位置、阶段身份或高阶公共能力展示，而且这个事实能解释“他为什么敢站在这里、别人为什么让路/忌惮、他为什么能守住某条线、为什么一个较弱者不能无视他”，Outline 不得把它从 Story Program 压缩掉；在这个人物第一次真正影响当前冲突或第一次展示该能力时，把“人物 = 精确位置/阶段 + 当前可见含义”保留进当前剧情块和 Future 10。不是人人报数字，也不为普通路人补级位；只保留已经批准、且会改变当前社会行为或行动因果的坐标。**一个明显高阶人物已经在场却没有介入他有能力改变的冲突时，同样需要读者能看懂为什么：优先保留上游已经批准的当前目标、职责边界、空间位置或利益边界，使“不出手”成为人物选择而不是作者忘记了他的力量。** 上游没批准具体原因时不得临时编造。
...
OUTLINE_STORY_ANCHOR_DENSITY_RULE = """Outline Story Anchor Density（软规划原则，不新增字段、评分或 Hard Gate）：中期大型剧情块不能只给一个覆盖十几二十章的阶段摘要，再把“中间具体发生什么”留给 Director 自己发明。每个剧情块在现有“具体发生”字段内，先用一句写清本块真正的故事主问题，再按因果顺序给出若干连续故事锚点；通常 3—5 个，只有很短的块可以 2 个，这只是内容密度参考，不是计数门禁。每个锚点是一次会改变局势的故事事件：写清谁为了什么采取行动、谁阻止或回应、主角做了什么关键选择或怎样使用核心优势、最后哪项关系/身份/力量/利益/行动空间发生不可逆变化，以及这个结果为什么触发下一锚点。**World Entry = Threat + Desire + Ruler**：重要新地点、势力、长期 Rival、传承或高价值机会第一次真正进入故事时，锚点先保留“为什么值得靠近/争、它相对主角当前位置有多重、谁已经为它付代价或冒险”，再安排必要危险与移动；不要让撤退路线、车队操作、登记或其它 supporting implementation 抢走入口本身。若后续要让主角放弃或失去某个高价值机会，必须先让读者知道它具体能带来什么、谁为它争、主角为什么会馋，否则机会成本只存在于计划里。长期 Rival 第一次进入时，如果 Approved World / Story 已有可用尺度，就用其层级、战绩、稀有能力、排名或真实位置中最相关的一项把他变成 Reader Ruler，不把 Rival 只写成态度合理的人。**Important Actor Carry-Forward 不只保护 Rival。** Story Program 若已经给当前窗口的重要配角明确的精确力量位置/高阶展示，或给出一条正在改变当前选择的私人因果（旧关系、失去的人、当前不愿承认的判断、旧案压力等），Outline 不得把人物压成“名字 + 任务”。在这个人物真正影响本块的相关锚点 / Future 10 中，只保留**当前会改变这一场选择、说话方式或是否介入的最小一层**：例如“这是亡侣留下的活物，所以她不能把这次争夺只当交易”，而不是整段 Biography；仍未 reader-facing 的隐藏旧史继续按 reveal 边界保护。**尤其是 Story Program 已经写成“具名人物 + 精确位置N + 在当前 Horizon/前期/某具名地点展示公共新动词X”的句子：这不是可被摘要成通用 Milestone 的背景，而是已批准的 reader-facing story causality。Outline 必须把三者绑定保留到该人物第一次实际进入对应事件的当前剧情块与 Future 10 `具体剧情`：人物名、精确位置N、展示X缺一项都算压缩丢失；不得只在 `## 0/## 2` 保留‘41级能共载 / 61级能择风’之类通用阶段说明。**

**Convergence is payoff, not simultaneous loading。** 多条重要人物线要在同一高负荷场景汇流时，读者应在真正全线碰撞前对其中大多数至少拥有一个可复述锚点：`他是谁 / 此刻要什么 / 为什么会来 / 若上游已有且与行为有关，他大致或精确处在哪个力量层`。这不要求每人提前一章，也不增加介绍配额；可以在同一剧情块前几个锚点里错峰让人物先各自做一件能立住自己的事。不要在第一次见面现场同时初始化三四个人的人生、目标、势力、力量与机关，然后立刻进入大汇流。

**复杂 Action Block 先冻结 Stable Scene Geography，再移动人物。** 如果一个连续冲突要跨多章反复使用多个路线、平台、门、锁点或高低位置，Outline 在当前块第一处真正复杂动作前，用一两句确定 3—5 个**后续会反复复用的稳定空间锚点及彼此关系**，只保留会影响选择/胜负的关系；后续 Future 10 用同一组名称描述人物移动。新空间点只有真的产生新行动可能时才增加，而且必须说明它相对已有锚点的位置；不能让每一章为了当前动作再临时发明一套“窄道 / 石台 / 斜坡 / 横梁 / 外侧”的局部舞台。锚点粒度停在“故事转折”，不要继续拆成路线选择、受力判断、搬运、制作、检测、计算、逐步验证等实施步骤；如果去掉操作过程后一个锚点就没有人物利益、力量差、关系变化、身份变化、资源得失、世界入口或不可逆结果，它就过于操作化。目标是提高故事确定性，不是提高施工步骤确定性。"""
...
PLANNING_TO_PROSE_BOUNDARY = """Planning → Prose 边界：Director、Plan、Canon 和 Audit 可以使用“验证、闭环、阶段推进、价值兑现、成长空间、建立优势”等抽象标签，也可以写“叙事功能、为什么这个选择重要、人物由此证明了什么”；这些是后台语义输入，不是正文必须复述的句子。不要把后台标签或策划层意义原样扩写成作者总结。先让人物、对象、动作、关系反应和具体结果承担意义；只有当前 POV 必须理解并据此选择时，才翻译成当下人物自然会想到的具体判断。**这里省掉的只能是已经被场景证明后的二次意义说明；不得因此省掉承载人物关系、情绪、价值权衡、社会反应或空间因果的具体动作、对白、停顿和生活纹理。**动作、对白或现实后果已经成立后，不再追加同义的抽象解释、主题判词或“这个选择意味着什么”的二次说明。"""
...
本轮实验的表达优先级是：清楚 > 顺畅 > 有画面 > 文学感。目标读者是普通中文男频网文读者；正文要让读者按正常速度读到一句时，基本立刻知道发生了什么。两种表达都准确时，选择普通、直接、一次读懂的说法，不为了“高级感”增加猜测成本。以下是 Primary Writer 的表达方向，不要求逐句自评或增加审稿阶段。

## 叙述按完整意思推进

把人物此刻看见、听见和已经明白的事情直接写出来。声音来源已明确，就直接交代来源及人物反应；没有实际误认，不先否定一个答案再揭晓另一个。转折应来自人物确实改变判断、发现新证据或反驳对方，普通观察不写成正反对句。

连续动作、感知和判断组成自然连贯的句段。一个自然段优先承载一个完整的“感知—动作—反应—判断” beat；不要因为“清楚”就把同一动作链切成一句一段。**段落连续性是重组句段，不是压缩故事：当前场景里已经成立的人物反应、对白交锋、关系摩擦、生活细节和会影响后续理解的动作 beat 仍要保留，只把属于同一 beat 的碎句组织得更连贯。**短独段只留给真正的冲击、危险突变、不可逆结果或情绪落点。已经发生的事由后果继续往下带，不再另起短句替它裁断或升华。不要把普通说明整理成整齐排比、重复句首或三项同构短句；各句写它真正需要承载的内容，长短随内容变化。
...
- **一次说话承载一个完整意思；对白长度由人物、关系和现场压力决定。** 同一人物正在说明条件、表达态度或追问理由，且没有被现场真正打断时，让他把这个意思说完，再由对方回应；不要把“短”当默认，也不要稳定生成“问一句—答四五个字—再问一句”的功能型乒乓。**让一个意思说完整不等于减少人物交锋：如果双方确实在试探、讨价、嘴硬、追问、拒绝或改变立场，保留足够的来回，让关系在说话过程中发生；不要为了“高效”把几轮有分量的互动压成一人一次立场申报。**短句应来自具体危险、真实打断、权力压力或这个人物本来的说话方式。简单应答、重复确认和次要交谈可以用叙述带过，保留真正有态度、关系或选择分量的原话。人物的恼火、迟疑、嘴硬、回避或亲近优先改变措辞、句长、停顿和是否答话，而不是另补情绪标签或为了制造情绪发明新的过去。
- **说话归属让读者直接跟住。** 多人交谈、第三人插话、话题转交或经过动作段后，及时用姓名、称呼或与人物绑定的动作交代说话人；不要让读者倒数引号猜轮次。同一人的话与他的动作可以放在同一自然段，两人明确轮换时可省略重复提示。人物动作要承载当前态度或意图，不为每句对白机械添加点头、抬眼、握拳。
...
5. 按意思与现场变化分段；同一人的一句话、接续动作和后半句话通常连在一段。只有真实的事件或情绪停顿需要独立落点时才单独成段，不为制造章末钩子把普通信息逐句拆开。
...
## 5. 配角与关系系统
写长期角色、各自利益、关系变化与反转、情绪价值，以及关系系统最容易失败的地方。关系可以承担崇拜、友情、师徒、竞争、嫉妒、恐惧、爱慕、忠诚、背叛和旧日轻视后的重新评价；不要把所有角色都写成制度利益方，也不要让所有人只做同一种反应。若上游 Story Program 已批准尚未 reader-facing 的人物旧史/隐藏关系，本节只保存当前表层关系与未解压力，不把完整后台答案复制进章节 Runtime。
## 6. 核心情节发动机
...
{LIVING_POWER_PROGRESSION_DIRECTION}
...
只保存 **0—6 条** Story Program 已经明确决定、而且很容易在 Outline 压缩时丢绑定的具名人物力量演示；没有就写 `NONE`。它不是新 Authority，也不是人物等级表，只是 Story Program 自己已经决定的 reader-facing causality 的结构化运输面。只有同时满足以下条件才列：①具名重要人物；②Approved World/Story 已有精确公开位置或阶段身份；③Story Program 已经决定此人在当前 Horizon 的具体阶段/地点提前展示某个公共能力/里程碑；④这次展示会改变主角期待、现场让路/忌惮、谁能封哪条路、或为什么这个人物有能力但选择不介入。

每条严格一行：
`- ACTOR-RULER-01｜人物：X｜精确位置：Y｜展示：Z｜时机/地点：当前阶段/具名地点或真实边界｜现场意义：一句具体影响`

**如果 Story Program 已经在正文自由段里决定了“唐某会以44级在某峡展示共载”，这里必须把同一决定结构化一次；不能为了少重复把这节省掉。** 反过来，不允许为了填这节临时给人物发等级、安排展示或创造场景。Outline 只能把这些 Anchor 放进其 `时机/地点` 允许的窗口，不能改演员、数字、公共能力或搬到更晚的另一个地点；若当前 Future 10 尚未到该边界，只保留在长期 Outline，不提前写进正文。
...
若 Approved Story Program 的 `### Reader-Facing Actor Ruler Anchors` 为 `NONE`，这里也只写 `NONE`。否则逐条**原样复制**每个 `ACTOR-RULER-xx`；不得删演员、改精确位置、换公共展示、改时机/地点或把它搬到另一个大型阶段。它不是新的 World Reader Release，也不是正文台词，而是防止 `Story Program → Outline → Future 10` 二次压缩丢失具名人物力量绑定的 schedule。当前 Future 10 落入某 Anchor 的 `时机/地点` 时，必须在该人物第一次真实进入对应冲突的 `具体剧情` 中兑现 `人物 + 精确位置 + 展示 + 现场意义`；若 Future 10 尚未到该边界，只保留 Anchor，不提前演示。
## 6. 核心情节发动机
...
## 9. 对话特点
写角色声音、潜台词、拒绝、回避、欲望和对话怎样改变现场；一次说话承载完整意思，多人交谈及时交代说话归属，次要应答可由叙述带过。
## 10. 节奏结构
...
叙事功能：写本章在局部故事中的作用。
结尾推动：写下一章为什么发生。
不可降格 Story Event：可选；只引用 BOOK `不可降格 Reader-Facing Story Event Registry` 中当前仍未发生、且本章真正到达排程边界的 `RSE-xx`。不能新增 ID、改写事件原子或把已经发生的 Event 再演一次。
...
### Power / Capability
**第一行固定维护且不得省略：** 若 World Root 有自然公开主尺名，写 `Current Power Position｜主尺：<精确主尺名>｜精确位置：<唯一数字位置>`；若 `主尺名称=NONE`，写 `Current Power Position｜精确位置：<唯一数字位置>`，**禁止把字面 `NONE` 带入 Canon 或章节上下文**。它是当前公开力量 Grammar 的坐标，不是总战力分。若本章没有明确改变精确位置，逐字保留输入中的 Current/Initial Position；只有正式正文明确让主角从一个精确位置进入另一个精确位置时才更新。**打赢更高等级对手、承受更高等级攻击、被人重新估价，都不能推断成等级本身变化。** 随后只写已经真实获得、证明或改变的能力/身体/武器使用能力与关键边界。**若正式正文已经直接建立新的成长因果或当前瓶颈，例如“某种 Asymmetry 被证明能减少伤停/提高有效训练”“当前精确位置没变但已经能稳定做到新的中间动作”“伤势暂时压低当前发挥”“某个最后门槛仍未跨过”，只把这条已经发生、以后会影响力量规划的事实压成最短一句保留；没有正文直接证据就不要推理。** 开局 Frozen Power Core 不在这里重写；这里承载后续 Power Delta、当前 Growth State 与 Advantage Stack 的已发生事实。若当前 Core 是累积同类变体型，优先按已成立的统一个人能力语法压缩保存，例如“已保留/掌握的同族性质有哪些、哪些组合已经真实成立”，不要把同一棵能力树拆成互不相关的技能包。**外部兵器、法宝、坐骑或普通装备的所有权仍归 Possession / Tracked Assets；只有已经真实学会的使用方式或与自身力量形成的复合能力才进入 Power / Capability。**
...
def _chapter_book_context(book_content: str) -> str:
    from .story_event_obligations import strip_book_registry_from_section
    ...
    for heading in headings:
        block = _extract_markdown_block(book_content, heading)
        if heading == "## 1. 核心类型与读者承诺":
            block = strip_book_registry_from_section(block)
        if block:
