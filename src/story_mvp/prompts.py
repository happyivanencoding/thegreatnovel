...
COMPOUNDING_GROWTH_DIRECTION = """COMPOUNDING_GROWTH_DIRECTION：成熟长篇不只让主角一次次解决更大的危机，还要让主要行动产生可保留、可再次利用的净新增。新增可以是能力、技艺、资源、装备、知识、路线、关系、身份、势力、领地、生产能力、规则权限或本书自己的其它资产；重要的是上一轮获得的东西真实进入下一轮，帮助主角取得过去无法取得的结果。临时、借用或条件型优势可以存在，但长期成长应表现出越来越强的留存、掌握、组合、控制或自主创造能力，而不是每轮结束重新归零。"""
