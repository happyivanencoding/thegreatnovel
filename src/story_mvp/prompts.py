...
开篇除了最低规则坐标，还要建立最低世界定向：读者大致知道“我现在在哪里、这里的人怎样活、谁拥有真正力量、当前生活边界之外通常怎样进入”。只释放当前故事真正需要的安全 / 社会 / 移动 / 世界坐标，不画百科地图。首次进入新的重要势力或地点时，优先在现有 `具体剧情` 中顺手写清“它是什么 / 为什么此刻重要 / 普通人或当前 POV 通常怎样理解它”；未获批准的真实原因、隐藏身份与长期谜底继续保持未知。
...
DIRECTOR_NARRATIVE_SALIENCE = """Director Narrative Salience：Director 面对十章计划和 Canon 中很多可执行细节时，八字段优先抓住本章最值得读的那一个人物冲突、关键选择、力量使用、反转、利益得失或兑现结果。操作步骤只有在它会改变人物选择、风险、胜负或结果时才进入“主角行动”；不要把“主角行动”写成施工单、路线单、检测流程或完整操作清单。**Discriminative Detail Only**：调查、手艺、医学、制作、检测、推理等 Supporting Skill 只保留足以改变判断的少量关键细节；一旦这些细节已经足以推出发现或决定，立即进入发现造成的选择、冲突或后果，不把实施过程继续展开成专业展示。**决定已经做完后的普通实施默认压缩**：除非实施中又出现新的选择、冲突、不确定性或失败风险，不把停车、搬运、排布、绑缚、登记、赶路等普通执行扩成主要场景。不能删改计划规定的结果，但可以把实现过程压缩到足够支撑因果的最小程度，让“谁逼谁、主角决定什么、局势怎样被改变”保持前景。"""
...
当 Curated Context 明确给出首次重要势力、地点、身份、资源或生活方式的已批准 Orientation Facts 时，允许用 1—3 个短的直接旁白段把读者此刻最需要的世界模型补齐：它是什么、为什么重要、当前 POV/普通人通常知道到哪里。**Action creates the question → Exposition answers just enough → Action continues.** 这类高价值 Orientation 可以替代低价值实施与重复验证，但不得越过 Canon / Plan 的未知边界，也不得顺手讲完整世界史。
