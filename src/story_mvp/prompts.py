...
精确位置是公开坐标，不是胜负公式；也不会因为越级战绩自动升级。**当 World 已批准有意义的大阶段名时，大阶段是读者的第一力量身份，精确数字是第二层校准**：正文优先形成“铸脊，炉级41 / 御风6重”这种可复述身份，而不是让“铸脊 / 御风”长期退化成后台区间名。真正跨过 Public Milestone 时，必须同时让读者看见那个阶段承诺的**新动词第一次成为现实**；只报“升到41级”而没有行动、环境、身份或世界入口变化，不算把大阶段的价值写出来。不要每章报表；重点在突破、越级、新圈层与重大 Public Proof 时让读者持续知道“到底什么大阶段、精确在哪一位、正常意味着什么、这次究竟靠什么偏离正常，以及下一次真正质变为什么值得等”。"""
...
- **规则说明随使用次数衰减。** Authority 可以完整、精确、重复列边界；正文不需要替 Authority 写说明书。某项独特规则/能力第一次真正影响选择时，用最短普通话讲清“它现在具体能做什么”；第二次使用只补**新的**边界、失败方式或组合变化；第三次以后若读者已经能从动作认出机制，优先只写动作 → 外界反应 → 结果，不再重述“它能A、不能B、也不能C”。新复合玩法第一次出现时，优先先让结果发生，让读者有机会自己意识到“原来还能这样接”，再在确有理解风险时补一句最短确认；不要在高潮前先把 `能力A + 能力B + 条件C = 结果D` 完整解释给读者。
...
### Power / Capability
**第一行固定维护且不得省略：** `Current Power Position｜主尺：<精确主尺名>｜精确位置：<唯一数字位置>`。它是当前公开力量主尺坐标，不是总战力分。若本章没有明确改变精确位置，逐字保留输入中的 Current/Initial Position；只有正式正文明确让主角从一个精确位置进入另一个精确位置时才更新。**打赢更高等级对手、承受更高等级攻击、被人重新估价，都不能推断成等级本身变化。** 随后只写已经真实获得、证明或改变的能力/身体/武器使用能力与关键边界。开局 Frozen Power Core 不在这里重写；这里承载后续 Power Delta 与当前 Advantage Stack 的已发生事实。若当前 Core 是累积同类变体型，优先按已成立的统一个人能力语法压缩保存，例如“已保留/掌握的同族性质有哪些、哪些组合已经真实成立”，不要把同一棵能力树拆成互不相关的技能包。**外部兵器、法宝、坐骑或普通装备的所有权仍归 Possession / Tracked Assets；只有已经真实学会的使用方式或与自身力量形成的复合能力才进入 Power / Capability。**
