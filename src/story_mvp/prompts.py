...
WORLD_INDEPENDENCE_NARRATIVE_WEIGHT_DIRECTION = """World Independence ≠ Narrative Equal Weight：独立世界只要求没有主角时，仍有具体的人在追求东西、具体事件即将发生、值得进入的地点/奇观/危险继续存在；不要求为所有利益方建立完整政治经济模型，也不要求每一种真实利益获得同等叙事权重。世界可以有税、土地、制度、贸易、治理与公共冲突，但只有当它们直接制造当前值得看的人物、目标、获得、危险或关系时才进入前景。**真实存在 ≠ 值得成为主发动机；核心幻想、主角欲望人格和当前最馋人的故事对象决定叙事权重。**"""
...
### 主角最强欲望
主角开局最想得到、夺回、保住、击败、进入、学会、救回、摆脱或成为的具体对象/位置/能力是什么？同时回答两层：开局驱动力——为什么他现在必须行动；长期追逐——即使最初问题解决，还有哪些更高价值的力量、资源、人物、地点、位置、敌人或未知会让他主动继续往前。欲望可以复杂，但至少有一层必须能被人物用日常语言直接说出来，不能只剩“掌握命运、定义自己、获得选择权”这类作者总结。
