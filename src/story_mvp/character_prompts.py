...
- **Progressive World Skeleton / Think Deeply, Freeze Sparsely。** 你可以从整本书尺度深思考世界，但开书 Authority 只冻结：World Root、当前自然 Horizon、当前已经在行动的 Living Actors、今天仍在产生后果的 Causal History，以及少量由本书题材自然长出的 Scale Promise。未来局部城镇、支线遗迹、市场/分舵、一次性资源、尚未进入当前因果的人物与旧史布景默认 OPEN，不为“完整”提前命名。**细节按因果需求加载，不按世界规模加载**：一旦继续增加事实已经不会改变当前人物的可行动选择、空间因果、价值判断、Reader orientation 或未来一致性，就停止具体化；未写出的部分不是缺陷，也不要建立大串 TBD。
- **Scale Promise Spine ≠ 必去路线。** 远方只冻结少量没有它整本书就会失去方向感或尺度承诺的 Landmark / Threshold / World Layer；节点由本书自己决定，可以是城市、宗门内部层、祖地、海沟、巨兽背城、文明中心、古路、世界树层、终极海域或任何更合适的形态，**不要求古墓、宗门、城市、禁区或星路**。每个远方节点只写“比上一层多打开什么世界 + 现在必须稳定的最少事实”，其内部人物、路线、遗迹、奖励与支线继续 OPEN。Final Scale 可以稳定，Final Boss / 最终传承 / 最后战场 / 终极谜底默认不因 Scale Promise 自动冻结。
...
- **精确力量主尺是强制 World Root Authority，但“精确”不等于再给整套尺造一个专名。** 每个 production World 必须让任何主要修炼者都有**唯一精确当前位置**，不能只有“低阶/中阶/高阶”或四五个粗境界。只允许三种简单语法：`连续数字`（如 1—100 级）、`大境界+数字子级`（如每境 1—9 星/重/层）、`数字序列`（如序列 9—0）。严格在 `## 力量体系与正常值` 下输出 `### 精确力量主尺｜Frozen Grammar`，逐行填写：`主尺类型`、`主尺名称`、`精确位置格式`（必须含 `{{N}}`）、`数字精度规则`（必须给阿拉伯数字范围）、`当前可见范围`、`当前大档位`。**`主尺名称` 默认写 `NONE`：像“化龙7重 / 43级 / 序列6”本身已经足够清楚时，不再创造“身天尺 / 九门尺 / 开身序 / 潮阶”这类只是在精确位置外再套一层标签的二次命名；只有社会本来就会反复说这个短称，而且它能减少解释而不是增加记忆负担时才填写。** 大境界名也属于稀缺概念：相邻两个阶段若没有清楚的身体结构变化、行动能力质变或社会层级变化，就合并，不为填满长阶梯继续造境界。Grammar 一旦批准就是 World Root：普通 macro World Expansion 只能向上延展可见范围，不能改成另一套计数法；真正独立 instance 可以拥有自己的本地精确尺，但不得改写全局主尺。**精确位置是 Reader Ruler，不是机械胜负公式，但也不能只是装饰数字。** 技能、装备、经验、环境、克制和 Power Asymmetry 仍可造成越级；同时主尺必须真实承载一组不会被单一特殊机制凭空抹掉的**基础盘**，例如力量/能量总量、身体承受、速度反应、作用范围、持续时间、可进入环境或本世界更自然的同类事实。特殊能力只自动改变它实际克制的那一部分；除非故事另有已经成立的多重因果，不能因为废掉高阶者一招，就默认连同高阶者其它主尺基础优势一起消失。不要因此建立总战力分、属性面板或逐项数值数据库。
...
- **Major Stage = Body Change = Action Space。** 对以人体成长为核心的玄幻，飞行、离体感知、耐寒/耐毒/缺氧、长距离御空、星空生存、横渡星海、影响山河等**基础行动权优先由大境界自身的身体/精神结构变化解释**，让读者看到“人变成了什么，所以世界少了一层限制”。高境界的新能力必须从前面已经建立的身体/精神 Grammar 继续长出来：例如内部循环更完整，所以能离开古星；自身运行足够稳定，所以能把已经掌握的力量方式压到更大外界范围。不要突然用“到王境便能控制任意法则”这种抽象万能权限替代连续因果。不要一边说某境已经能飞/能在虚空生存，另一边又让普通飞行、耐环境或离星必须依赖一套平行的资格、材料、职业或器械系统。身份可以限制合法进入，器物/阵法/异兽可以让低阶者借用一次、走得更远、更省力，特殊禁区也可以有额外条件；但这些只能是例外、捷径或特殊路线，不能抢走主力量尺对普通行动空间的解释权。
...
## 世界正在发生的大事
只写**当前自然 Horizon 已经在动**、即使未来主角从未出生仍会推进的少数事件，通常 2—4 件即可。每件先回答：**谁现在私人地想要什么 → 下一步已经准备做什么可见动作 → 没有主角也会让谁/哪件东西/哪个地方真实改变。** 至少一件允许由钱、赢、爱、嫉妒、占有、好奇、报复、舍不得、证明自己等非治理欲望发动；也允许异兽、族群或自然生命按自己的需要行动。若多件大事自然碰撞，优先让它们因为争同一个具体人/物/地点、抢先抵达、带走/毁掉某物或互相追杀而相撞，不要自动写成多方协调资源。不要为此新增角色表、倒计时表或事件 schema。不要为了展示世界丰富度再铺远方支线事件；当前 Horizon 之外留给后续 World Expansion。

## 已经活过的人与关系史
只写少量真正会改变**当前 Horizon**行动的既有人物关系结；没有足够好的就写 `NONE`。每个用普通话写清：**过去的关键选择/关系 → 今天留下的具体残留 → 现在至少两个人因此分别会做什么**。可以是父母与子女、旧爱、师徒、旧敌、朋友、救命/欠债、共同失败、背叛、继承、失踪者留下的空位，也可以自然跨一代。**冻结因果，不冻结无关布景**：如果一个高价值旧选择本来会同时压住第三个人或下一代，就保留谁继承了身份/误解/债/遗物/路线，以及这个人今天因此怎样行动；但当年的河谷、客栈、日期、同行小遗迹等若今天不改变因果，就不要决定。人物今天仍可以彼此误解或只知道部分真相；若某段历史值得长期重释，保留一个能长期成立的表层解释和一个尚未完全 reader-facing 的更深因果。不要把所有关系都连起来，不写完整生平，不预设未来主角与他们的关系。

## 值得进入的地点、奇观与未知
只写当前 Horizon 真正需要的少量稳定地点、signature 奇观、危险与未知；它们不是为某个尚不存在的能力准备的插座，也不要预制一排未来支线地点。

## Scale Promise Spine｜Adaptive Landmarks
从当前生活尺度到本书已知最高尺度，只列少量真正构成长期方向感的 Landmark / Threshold / World Layer。节点由本书题材自己长出来，不套“小镇→大城→宗门→古墓→星路”模板，也不要求任何固定类别。每条固定写：`<节点>`｜尺度变化：`比上一层多打开什么世界`｜现在必须冻结：`1—3条以后不能随便改的根事实`｜保持 OPEN：`当前没必要决定的内部人物/路线/遗迹/奖励/支线`。Scale Promise 不是必去路线，不给每层自动配置奖励、守关人或主角任务。

## Deferred World｜Not Yet Canon
只用一个短段说明哪些**类型的局部细节**刻意不在开书冻结、以后由 World Expansion 或合法 Story Backfill 在真正需要时生成。不要列几十个 TBD；未写出就默认 OPEN。
...
- **Progressive Detail Loading 同样适用于每次 Expansion。** 本轮只把新 Horizon 已经需要的人、地点、社会现实、价值物、Causal History 与少数 signature 奇观变高清；更远一层只保留必要 Scale Promise，不能因为已经开始扩世界就顺手把下一批城市、副本、遗迹、市场、人物和奖励全部生成。**停止规则与 Opening World 相同**：一旦继续增加细节已经不会改变当前人物可行动选择、空间因果、价值判断、Reader orientation 或已冻结世界的一致性，就停止，剩余默认 OPEN。
- **Small Grammar 继续累积，不因换地图重置。** 新城市、新大陆、新文明优先复用 Root 已学会的力量词、普通类别词与行动能力；少数新专名只给会反复出现且真正值得记住的奇观、势力、人物或高价值物。不要让每个新地点都重新出现一套能量、资格、材料、测量名或环境权限。
...
- `instance` 必须像一个原本就在运行的小世界：有普通生活、当地力量/危险、社会关系、价值物、人物欲望、正在发生的冲突与自己的未知；不是任务房、Boss 房或为某能力准备的谜题。独立副本可以有不同的本地力量语言，但也**强制拥有自己的精确本地主尺**：在 `## 新力量 / 威胁 / 身份 / 价值尺度` 内输出 `### 本地精确力量主尺｜Instance Grammar`，至少写三选一的 `主尺类型`、`主尺名称`、含 `{N}` 的 `精确位置格式`、明确数字范围的 `数字精度规则`、`当前可见范围`、`与全局主尺关系`。**本地主尺名称也默认 `NONE`**；像“海兽23级 / 序列6 / 赤阶4重”本身足够清楚时，不再为了副本气味套第二层名字。本地尺只帮助读者理解该世界，不回写全局主尺。若本地尺有真正的大阶段质变，同样补 `### 本地公共力量里程碑｜Instance Ladder`，只列当前副本真实可见的少数公共节点，不为副本填百科；公共能力继续用直接动作句，不造能力短名。
...
## 新地点、势力与公共识别
只具体化本轮新 Horizon 已经会被人物行动触碰的少量地点、公共势力和稳定空间锚点；通常只需要 3—5 个能长期复用的空间/势力锚点。普通运输组织、维修协作体、路线工具、材料类别若只是支撑生活，优先用普通类别名，不为世界完整逐一命名；不要顺手生成下一层支线地图。
## 世界人物欲望与正在发生的事
只写当前新 Horizon 已经在动、没有主角也会推进的少数事件，通常 2—4 件即可。至少有一条主要欲望必须直接指向本 Horizon 的 Fantasy Surface——想进入哪里、得到/夺回什么、见到什么强者/生物、参与什么高价值冲突或揭开什么可见异常；**不能让整段新世界主要靠修路、护运、维修、配给、路线管理或多方协调来发动。**
## 新人物的既有关系与旧选择
只写会改变当前行动的少量关系结：过去选择/关系 → 今天残留 → 当前各自行动；没有就写 `NONE`。**冻结因果，不冻结无关布景**：旧事的具体客栈、河谷、日期、沿途小遗迹等若当前无因果价值就继续 OPEN。可跨代，但不画关系网，不知道未来主角。
## 真正值得想要或进入的东西
只保留会改变本轮人物欲望、路线或价值判断的少量高价值对象/入口。优先写功法、兵器、身体变化、异兽/伙伴、传承、强者、奇观、身份跃迁或本书自己的主要幻想对象；普通通行份额、维修方法、运输工具、配给资格等 supporting object 只有它本身已经成为当前高价值欲望时才前景化。未来可能有用但当前无关的对象不预制。
## 下一层 Scale Promise｜Adaptive Landmarks
只写从本轮 Horizon 再往外看时，少量真正构成尺度承诺的 Landmark / Threshold / World Layer。节点由题材自己决定，不套固定地点模板。每条写：`<节点>`｜尺度变化：`多打开什么世界`｜现在必须冻结：`最少根事实`｜保持 OPEN：`内部人物/路线/遗迹/奖励/支线`。它不是主角必去路线，也不自动生成下一次 Expansion。
## Deferred World｜Not Yet Canon
一个短段说明本轮刻意不具体化的未来局部细节类型；未写出的内容默认 OPEN，不列大串 TBD。
## 仍未知的边界
"""
