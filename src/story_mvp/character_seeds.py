...
先说明与本优势直接相关的世界力量正常值、稀缺度或获得/承载规则，再说明持有者相对同层普通人/天才到底多了什么明显特权。来源不必强制是世界内的合法例外，也可以是唯一奇物、外来知识/经验、外挂、极端天赋或少量优势叠加；故事开始时可以暂时解释不全，但可见结果必须清楚。默认宁强勿弱：至少一个维度应强到让同层人明显羡慕，边界防万能但不把纯收益抵消掉。若 World 有公开精确主尺，同时用普通话保留“主尺负责哪些基础盘、Asymmetry 改变哪一部分”的真实耦合：局部高阶特权可以提前出现，但不能自动把明显更高档对手的其余主尺基础优势全部清零。
...
- 正常成长耦合｜Growth Coupling：先写 World 的正常成长瓶颈，再明确 Power Asymmetry 是否改变其中某一环；若改变，说明它怎样改变学习/吸收/承受/恢复/资源效率/实战反馈/突破条件或可接触机会，以及为什么会让成长更快、更稳或上限不同；若没有真实关系，固定写 `不改变正常修炼速度`。
...
## 世界中的初始位置与生活事实
第一行固定保存开局精确位置：若 World Root `主尺名称=NONE`，写 `开局精确力量位置｜精确位置：<明确数字位置>`；只有 World Root 真有自然公开主尺名时，才写 `开局精确力量位置｜主尺：<主尺名>｜精确位置：<明确数字位置>`。**禁止把字面 `NONE` 写进 Human / Character。** 这是人物在世界公开力量 Grammar 上的 T0 坐标，不是 Power Asymmetry，也不会随以后升级被回写。其余只来自 LIFE_CONTEXT；写具体出身、家庭、教育、工作/修炼接触与真实发生过的生活事实。**经历是生活上下文，不是人格证明**：不要求每段经历都逐条推出一个人格适应结论，也不从预先决定的人格反向发明恰好证明它的童年。
...
如果上面的“世界中的初始位置与生活事实”里已经**独立成立**了某个具体旧物、家庭位置、反复发生的生活经验、长期失去/占有经验、羞辱、关系习惯或其它现实，而它自然会给当前最强的一条选择偏向增加重量，可以补一句 `生活根：...`，说明这个偏爱为什么对他特别刺。**只能引用上面已经先写出的生活事实，不得为了证明 Behavior Signature 反向新造童年、创伤或悲惨身世。** 没有自然生活根就不写；人物可以暂时连自己都说不清为什么在意。
