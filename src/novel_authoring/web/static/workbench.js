(function () {
  ...
  function bindCandidateDraft(current) { current.querySelectorAll("[data-selected-candidate-draft]").forEach(function (button) { button.addEventListener("click", function () { var candidateId = button.dataset.selectedCandidateDraft || ""; var base = "/api/books/" + encodeURIComponent(current.dataset.bookId) + "/editions/" + encodeURIComponent(current.dataset.editionId) + "/planning/candidates/" + encodeURIComponent(candidateId) + "/draft"; button.disabled = true; button.textContent = "正在核对并准备任务…"; postJson(base, { candidate_id: candidateId, edition_id: current.dataset.editionId }).then(function (result) { button.textContent = "正文任务已准备"; var feedbackNode = button.parentElement.querySelector("[data-selected-candidate-feedback]"); if (feedbackNode) feedbackNode.textContent = "已生成 Chapter Contract 与 Draft handoff，请在任务中心执行。"; var handoff = result.handoff || {}; if (handoff.handoff_id) { var url = new URL(location.href); url.searchParams.set("activity_id", handoff.handoff_id); setTimeout(function () { loadWorkbench(url.href, { push: false, restoreState: Object.assign(captureNavigationState(current), { activityCenterOpen: true }) }); }, 350); } }).catch(function (error) { button.disabled = false; button.textContent = "选择并准备正文任务"; var feedbackNode = button.parentElement.querySelector("[data-selected-candidate-feedback]"); if (feedbackNode) feedbackNode.textContent = error.message; }); }); }); }
  ...
  function initWorkbench(current) { bindLayout(current); bindNavigation(current); bindScrollPersistence(current); bindActivityCenter(current); bindCommands(current); bindInspector(current); bindRelationshipGraph(current); bindStateWorkspace(current); bindItemModal(current); bindProfile(current); bindProgressionContracts(current); bindTruth(current); bindSecretBoard(current); bindDraft(current); bindCandidateDraft(current); bindWorkflow(current); bindPendingActions(current); bindSearch(current); current.querySelectorAll("[data-copy-instruction]").forEach(bindInstructionCopy); current.querySelectorAll("details[data-explorer-section]").forEach(function (item) { item.addEventListener("toggle", function () { saveFallback(current, captureNavigationState(current)); }); }); }
