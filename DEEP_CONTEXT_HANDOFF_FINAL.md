...
Windows 持久 `OPENAI_API_KEY` 现由 production 统一解析：`当前进程 → User Environment → Machine Environment`。`gbrain.py` 会在宿主进程未继承 Key 时把解析到的持久 Key 显式传给 GBrain 子进程，`gbrain_retrieval._semantic_query_available()` 也使用同一 resolver；Key 不写 repo / `.env`。**凡 production 阶段配置为 GBrain ON，embedding Key 现在是硬前置：resolver 找不到 Key 时 `/api/gbrain/brief` / `/api/gbrain/query` 立即 fail loud 并停止生成，不再允许 keyword-only 降级或通用卡补位。** GBrain OFF 的 Primary / Batch Authority Delta / State 等阶段不受影响。

Retrieval ownership 专项已扩大到 7 组有效质量样本：宁烬真实 Horizon 的 Sol-high A/B，以及游戏副本、亚特兰蒂斯、兽脊三种 Frozen Human、坠星海共 6 组 Terra-high screening。Integrated JIT Story Agent 出现 >15 分钟无 final，记运行 FAIL，不进入质量票。原实验观察到六个新增 Story Program 要么 semantic accepted=0，要么 keyword fallback **6/6 收敛到同一组三张**；继续根因追踪后发现一个更早的实现 bug：Windows `gbrain.cmd` 不能安全承载多行位置参数，旧 TGN 把多行 query 直接传入时会在首行截断，并可能把后面的 `--scope` 一起丢掉。这解释了“不同书 query 分数完全一致”和非小说 scope 污染。现在 `query_gbrain()` 在 CLI 边界把所有换行/连续空白压成单行。
...
## 2026-09-04｜Progressive World Skeleton 正式冻结：Think Deeply, Freeze Sparsely

随后针对 Sol-max“世界设计质量高但开书信息量/未来 Canon 过满”的问题做 matched A/B。相同作者方向、相同新版低词汇 World Prompt、相同 GBrain、相同 Sol-max：A Full Baseline **890.27s / 8066 chars / 76549 tokens**，在当前石梁城/沉星井之外已提前冻结暖脊高原、折兵月、无灯星门、熄光海及对应人物、争夺和三十七年前完整星门旧史；B2 Progressive Skeleton **765.28s / 7798 chars / 61127 tokens**，保住完整 Power Root、Living Actors、Causal History、宇宙尺度与奇观，却把当前具体世界收回到石灯城/落月原/回光裂谷/残月环，远方只保留 `苍衡古星 → 跨星古路与近星域 → 中央星海 → 无昼星墟与宇宙边荒` 的 sparse Scale Promise，每层只冻结尺度变化 + 1—3 条根事实 + OPEN 边界。B1 曾因 experimental override 新增第二套一级输出 Schema 导致只输出 Scale Spine，明确判 **INVALID**；最终 production 保留原 World schema，只调整冻结范围。

跨题材验证也通过：海洋驭兽 held-out Luna-high **263.04s / 5489 chars / 57715 tokens** 自然生成 `白舷港—近岸浮岛群 → 巨型海兽背城与迁徙带 → 深海裂谷与沉没文明层 → 星外海洋`，证明 Scale Promise Landmark 由题材自己的尺度承诺生成，不套“小镇→大城→宗门→古墓→星路”模板。正式规则因此冻结为 **Progressive World Skeleton / Think Deeply, Freeze Sparsely**：Opening World 只冻结 `World Root + Current Horizon + Current Living Actors + Causal History + Adaptive Scale Promise + Deferred/Open World`；未来局部城市、副本、支线遗迹、市场/分舵、一次性资源、未进入当前因果的人物与旧史布景默认 OPEN。停止点不是地点配额，而是因果充分性：继续加事实若已不会改变当前可行动选择、空间因果、价值判断、Reader orientation 或未来一致性，就停止。Causal History 冻结因果，不冻结无关河谷/客栈/日期/沿途小遗迹；Final Scale 可稳定，但 Final Boss / 最终传承 / 最终战场 / 终极谜底不因 Scale Promise 自动定真。Scale Promise / Deferred World 是 planning/backstage Authority，章节 safe `WORLD AUTHORITY` 明确排除，只有后续 World Expansion 真正把某块 OPEN 区域具体化为当前 Horizon 后才进入 Writer。

同一原则同时正式接入 `WORLD_EXPANSION_PROMPT`：每次 Expansion 只把本轮真实到达的新 Horizon 变高清，下一层继续 sparse Scale Promise，Small Grammar 跨地图累积，不因换城市/大陆/副本重新造能量、资格、材料、测量名或行动权限；独立 instance 的本地主尺名也默认 `NONE`。第一条真实 production-style Expansion smoke（Luna-high **202.61s / 3650 chars**）虽然 Sparse Future 正确，却让当前 Horizon 被三河门/长风廊/渡风社、货运份额、航标维修、配给与路线工具占满，因此没有勉强判 PASS，而是继续收口：当前新 Horizon 通常只需 3—5 个可复用空间/势力锚点、2—4 件无主角也会推进的事件，并至少有一条主要欲望直接指向奇观、强者、生物、传承、兵器、身体变化或本书自己的 Fantasy Surface；修路、护运、维修、配给、路线管理只能做 supporting logic。V2 使用完全相同 Root/Horizon 输入，Luna-high **240.29s / 2736 chars / 56803 tokens**，转为 `七脊河原 / 悬脊城 / 沉云井 / 天坠坞`，主冲突由失踪同伴、封井、旧星舟遗产、云背兽幼兽发动；再下一层只保留 `七脊河城带 / 天坠坞深层` 两个 sparse Promise，内部继续 OPEN，判 **PASS**。
...
用户进一步明确：**final-output-only / hands-off 不是一个可选模式，而是 TGN 默认产品工作方式。** 用户不会逐阶段分析 World / Character / Story / Expansion，也不会中途做候选批准；主要输入是方向、已有冻结材料（若有）、目标章数，主要输出是最终正文，反馈发生在阅读体验层。今后 ChatGPT-operated 长任务在初始方向明确后，由 TGN operator 按现有 production 规则、冻结输入、validator 与预先选择原则完成内部 candidate selection / Save / Adopt / Freeze checkpoint 与正常失败恢复。`author_approved` 等旧状态名暂保留兼容，但语义是内部 Frozen Authority，不再等于“用户必须点击”。Author Workspace 的审批密集创意面降为高级检查 / 人工干预，Overview 以 Automatic Production Runs 为前台；不要因为自动化再造 Recovery Controller / Judge / Reviewer。
