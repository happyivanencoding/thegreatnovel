# Local Authority Repair L1｜Frozen Derivation Protocol
...
- If no center survives, L1 is unsupported and the real route would fallback high.
...
`Primary` vs `L1 Local Repair` vs `Luna-high Full Reviser`.

L1 may advance to a third entirely new held-out novel only if all are true:

1. L1 Story mean is within 2 points of both Primary and high, with no catastrophic truncation.
2. L1 Authority hard-problem count <= high + 2 across the 8 samples, and Authority mean is within 3 points of high.
3. All unselected paragraphs remain exact; paragraph count unchanged; patch parse/apply 8/8.
4. Mean L1 repair wall is at least 40s/chapter lower than high Reviser wall on the same stored sample set.
