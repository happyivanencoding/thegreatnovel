...
ROOT = Path('books/real-exp-appetite-diversity-three-20260825-v1')
DIRECTION = (
...
def materialize_seed() -> None:
    text = clean_acp(ROOT / 'seed_acp.md')
...
def materialize_worlds() -> None:
    for i in (1, 2, 3):
        p = ROOT / f'book-{i}'
        (p / 'WORLD_VISION.md').write_text(clean_acp(p / 'world_acp.md'), encoding='utf-8')
...
def materialize_programs() -> None:
    for i in (1, 2, 3):
        p = ROOT / f'book-{i}'
        (p / 'STORY_PROGRAM.md').write_text(clean_acp(p / 'program_acp.md'), encoding='utf-8')
