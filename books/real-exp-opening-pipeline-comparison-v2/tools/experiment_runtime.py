...
EXP = REPO / "books" / "real-exp-opening-pipeline-comparison-v2"
SRC = REPO / "src"
...
def main() -> None:
    ...
    parser.add_argument("command", choices=("reset-book", "init", "render", "record", "select", "finish", "adopt-integrator", "finalize", "apply-state", "execution"))
    parser.add_argument("candidate", choices=tuple(CANDIDATES))
    parser.add_argument("chapter", type=int, nargs="?")
    parser.add_argument("value", nargs="?")
    args = parser.parse_args()
    if args.command != "reset-book" and (args.chapter is None or args.chapter < 1 or args.chapter > 3):
