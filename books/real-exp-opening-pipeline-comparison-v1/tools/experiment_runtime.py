"""Opening pipeline comparison 的最小实验编排器。
...
EXP = REPO / "books" / "real-exp-opening-pipeline-comparison-v1"
SINGLE_EXP = REPO / "books" / "real-exp-opening-three-chapter-hook-v1"
SRC = REPO / "src"
...
def render(candidate: str, chapter: int, mode: str) -> Path:
...
def write_execution_record(candidate: str, chapter: int) -> Path:
    ...
    executed_specialists = [
        name
        for name in SPECIALISTS
        if manifest["nodes"].get(name, {}).get("status") not in {"skipped", "missing"}
    ]
    ordered = [
        "director",
        "chapter_prep",
    ...
    record = {
        ...
        "selection_mismatch": executed_specialists != selected,
...
def _chapter_bundle(root: Path) -> str:
    chunks: list[str] = []
    for chapter in range(1, 4):
        body = read(root / "chapters" / f"chapter-{chapter:04d}.md").strip()
        if not body:
            raise ValueError(f"盲读包缺少 Chapter {chapter} 正文：{root}")
        chunks.append(f"## 第{chapter}章\n\n{body}")
    return "\n\n".join(chunks) + "\n"


def prepare_blind_inputs() -> None:
    """生成只含正文的交叉 A/B 盲读包；映射单独保存，不进入 Reader Prompt。"""

    review_root = EXP / "reviews" / "blind-reader-input"
    placements = {
        "candidate-b": {"text-a": "hybrid", "text-b": "single"},
        "candidate-c": {"text-a": "single", "text-b": "hybrid"},
    }
    key_lines = ["# Blind Reader Key", "", "映射只供主线程与系统复核使用，不喂给盲读 Reader。", ""]
    reader_prompt = """你是一个不知道 Writer Mode 的盲读 Reader。只读取本目录的 `text-a.md`、`text-b.md` 和本目录的三章正文；不要读取上级目录、key、Prompt、Run、Review、manifest、版本或任何其它文件。
...
"""
    for candidate, placement in placements.items():
        pair = review_root / candidate
        write(pair / "text-a.md", _chapter_bundle(
            candidate_dir(candidate) if placement["text-a"] == "hybrid" else SINGLE_EXP / candidate
        ))
        write(pair / "text-b.md", _chapter_bundle(
            candidate_dir(candidate) if placement["text-b"] == "hybrid" else SINGLE_EXP / candidate
        ))
        write(pair / "reader_prompt.md", reader_prompt)
        key_lines.extend(
            [
                f"- {candidate}: text-a={placement['text-a']}; text-b={placement['text-b']}",
                f"  Single source: `{SINGLE_EXP / candidate / 'chapters'}`",
                f"  Hybrid source: `{candidate_dir(candidate) / 'chapters'}`",
                "",
            ]
        )
    write(review_root / "blind-reader-key.md", "\n".join(key_lines))


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("command", choices=("reset-book", "blind-pack", "init", "render", "record", "select", "finish", "adopt-integrator", "finalize", "apply-state", "execution"))
    parser.add_argument("candidate", choices=tuple(CANDIDATES))
    parser.add_argument("chapter", type=int, nargs="?")
    parser.add_argument("value", nargs="?")
    args = parser.parse_args()
    if args.command not in {"reset-book", "blind-pack"} and (args.chapter is None or args.chapter < 1 or args.chapter > 3):
        raise SystemExit("本实验只允许 Chapter 1—3")
    if args.command == "reset-book":
        reset_book_to_clean_state(args.candidate)
